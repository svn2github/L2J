...
import l2next.commons.net.utils.NetUtils;
 ...
 * <p/>
 * c: server list size (number of servers) c: last server [ (repeat for each servers) c: server id (ignored by client?) d: server ip d: server port c:
 * age limit (used by client?) c: pvp or not (used by client?) h: current number of players h: max number of players c: 0 if server is down d: 2nd
 * bit: clock 3rd bit: wont dsiplay server name 4th bit: test server (used by client?) c: 0 if you dont want to display brackets in front of sever
 * name ]
 * <p/>
 * Server will be considered as Good when the number of online players is less than half the maximum. as Normal between half and 4/5 and Full when
 * there's more than 4/5 of the maximum number of players
 */
			...
			if(gs.getAdvancedIps() != null)
			{
				for(AdvancedIp advancedIp : gs.getAdvancedIps())
				{
					if(!added && NetUtils.CheckSubNet(client.getIpAddress(), advancedIp))
					{
						try
						{
							_servers.add(new ServerData(gs.getId(), InetAddress.getByName(advancedIp.getIpAdress()), gs.getPort(), gs.isPvp(), gs.isShowingBrackets(), gs.getServerType(), gs.getOnline(), gs.getMaxPlayers(), gs.isOnline(), entry == null ? 0 : entry.getKey(), gs.getAgeLimit(), entry == null ? ArrayUtils.EMPTY_INT_ARRAY : entry.getValue()));
							added = true;
						}
						catch(Exception e)
						{

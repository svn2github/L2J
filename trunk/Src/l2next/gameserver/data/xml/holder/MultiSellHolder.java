...
import l2next.gameserver.network.serverpackets.MultiSellList;
...
import l2next.gameserver.utils.XMLUtil;
...
public class MultiSellHolder
{
	...
	private void parse()
	{
		...
		//files = new ArrayList<>();
		//hashFiles("custom/multisell/community/" + Config.communityFolder, files);
		//for(File f : files)
		//{
		//	parseFile(f);
		//}
		...
		if(entry.getIngredients().size() == 1 && entry.getProduction().size() == 1 && entry.getIngredients().get(0).getItemId() == 57)
		{
			...
			/*if (item.getReferencePrice() > entry.getIngredients().get(0).getItemCount() && !Config.LOGIN_SERVER_IS_PVP)
				_log.warn("MultiSell [" + multiSellId + "] Production '" + item.getName() + "' [" + entry.getProduction().get(0).getItemId() + "] price is lower than referenced | "
				        + item.getReferencePrice() + " > " + entry.getIngredients().get(0).getItemCount());*/ //Awakeninger: ОЧень засирает лог
		}
	...
	private MultiSellListContainer generateMultiSell(MultiSellListContainer container, Player player, double taxRate)
	{
		...
		for(MultiSellEntry origEntry : container.getEntries())
		{
			...
			// Адены добавляются в лист если отсутствуют или прибавляются к
			// существующим
			...
			if(!notax && taxRate > 0.)
			{
				...
				for(MultiSellIngredient i : ent.getIngredients())
				{
					...
					// FIXME hardcoded. Налог на клановую репутацию. Формула
					// проверена на с6 и соответсвует на 100%.
					// TODO Проверить на корейском(?) оффе налог на банг
					// поинты и fame
					{
			...
			// Если стоит флаг "показывать все" не проверять наличие
			// ингридиентов
			if(showall)
			{

...
import l2next.commons.geometry.Rectangle;
import l2next.commons.geometry.Shape;
...
public class ZoneParser extends AbstractDirParser<ZoneHolder>
{
	...
	public ArrayList<File> getXMLDir()
	{
		...
		files.add(new File(Config.DATAPACK_ROOT, "data/zone/"));
		return files;
	}
	...
	public static Polygon parsePolygon(Element shape) throws Exception
	{
		...
		for(Iterator<Element> i = shape.elementIterator(); i.hasNext(); )
		{
			...
			{
				String[] coord = d.attributeValue("loc").split("[\\s,;]+");
				if(coord.length < 4) // Не указаны minZ и maxZ, берем граничные
				// значения
				{
	...
	public static Circle parseCircle(Element shape) throws Exception
	{
		...
		if(coord.length < 5) // Не указаны minZ и maxZ, берем граничные
		// значения
		{

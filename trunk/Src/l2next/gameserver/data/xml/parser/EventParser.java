...
import l2next.gameserver.model.entity.events.actions.ActiveDeactiveAction;
import l2next.gameserver.model.entity.events.actions.AnnounceAction;
import l2next.gameserver.model.entity.events.actions.GiveItemAction;
import l2next.gameserver.model.entity.events.actions.IfElseAction;
import l2next.gameserver.model.entity.events.actions.InitAction;
import l2next.gameserver.model.entity.events.actions.NpcSayAction;
import l2next.gameserver.model.entity.events.actions.OpenCloseAction;
import l2next.gameserver.model.entity.events.actions.PlaySoundAction;
import l2next.gameserver.model.entity.events.actions.RefreshAction;
import l2next.gameserver.model.entity.events.actions.SayAction;
import l2next.gameserver.model.entity.events.actions.SpawnDespawnAction;
import l2next.gameserver.model.entity.events.actions.StartStopAction;
import l2next.gameserver.model.entity.events.actions.TeleportPlayersAction;
import l2next.gameserver.model.entity.events.objects.BoatPoint;
...
import l2next.gameserver.model.entity.events.objects.DoorObject;
import l2next.gameserver.model.entity.events.objects.FortressCombatFlagObject;
import l2next.gameserver.model.entity.events.objects.SiegeToggleNpcObject;
import l2next.gameserver.model.entity.events.objects.SpawnExObject;
...
public final class EventParser extends AbstractDirParser<EventHolder>
{
	...
	public ArrayList<File> getXMLDir()
	{
		...
		files.add(new File(Config.DATAPACK_ROOT, "data/events/"));
		return files;
	}
	...
	{
		for(Iterator<org.dom4j.Element> iterator = rootElement.elementIterator("event"); iterator.hasNext(); )
		{
			...
			try
			{
				eventClass = (Class<GlobalEvent>) Class.forName("l2next.gameserver.model.entity.events.impl." + impl + "Event");
			}
			...
			for(Iterator<org.dom4j.Element> parameterIterator = eventElement.elementIterator("parameter"); parameterIterator.hasNext(); )
			{
			...
			if(onTime != null)
			{
				for(Iterator<org.dom4j.Element> onTimeIterator = onTime.elementIterator("on"); onTimeIterator.hasNext(); )
				{
			...
			for(Iterator<org.dom4j.Element> objectIterator = eventElement.elementIterator("objects"); objectIterator.hasNext(); )
			{
	...
	private List<Serializable> parseObjects(Element element)
	{
		...
		for(Iterator<org.dom4j.Element> objectsIterator = element.elementIterator(); objectsIterator.hasNext(); )
		{
			...
			else if(nodeName.equalsIgnoreCase("siege_toggle_npc"))
			{
				...
				for(Iterator<org.dom4j.Element> oIterator = objectsElement.elementIterator(); oIterator.hasNext(); )
				{
	...
	private List<EventAction> parseActions(Element element, int time)
	{
		...
		for(Iterator<org.dom4j.Element> iterator = element.elementIterator(); iterator.hasNext(); )
		{

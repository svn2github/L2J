package l2next.gameserver.data.htm;
 ...
 *
 * @author G1ta0
 * @reworked VISTALL В кеше список вот так admin/admhelp.htm admin/admin.htm admin/admserver.htm admin/banmenu.htm admin/charmanage.htm
 */
public class HtmCache
{
	...
	public static final int ENABLED = 2; // кеширование отключено (только для
	// тестирования)
		...
		switch(Config.HTM_CACHE_MODE)
		{
			case ENABLED:
				for(Language lang : Language.VALUES)
				{
					...
					root = new File(Config.DATAPACK_ROOT, "data/html/scripts/services/community");
					if(!root.exists())
					{
						_log.info("HtmCache: Not find html dir for lang: " + lang);
						continue;
	...
	{
		Language lang = player == null ? Language.MULTI : player.getLanguage();
		...
		if(cache == null)
		{
			switch(Config.HTM_CACHE_MODE)
			{
				...
				case LAZY:
					...
					if(cache == null && lang != Language.MULTI)
					{
						cache = loadLazy(Language.MULTI, file);
					}
					break;
				case DISABLED:
					...
					if(cache == null && lang != Language.MULTI)
					{
						cache = loadDisabled(Language.MULTI, file);
					}
					break;
			}
		}
	...
	private String get(Language lang, String f)
	{
		...
		if(element == null)
		{
			element = _cache[Language.MULTI.ordinal()].get(f);
		}

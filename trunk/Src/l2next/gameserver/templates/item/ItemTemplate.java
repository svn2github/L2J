...
public abstract class ItemTemplate extends StatTemplate
{
	...
	{
		NONE(CRYSTAL_NONE, 0), D(CRYSTAL_D, 1), C(CRYSTAL_C, 2), B(CRYSTAL_B, 3), A(CRYSTAL_A, 4), S(CRYSTAL_S, 5), S80(CRYSTAL_S, 5), S84(CRYSTAL_S, 5), R(CRYSTAL_R, 6), R95(CRYSTAL_R, 6), R99(CRYSTAL_R, 6);
	...
	public enum SoulshotGrade
	{
		SS_NG, SS_D, SS_C, SS_B, SS_A, SS_S, SS_R;
	}
	...
	private List<CrystallizationItem> _exCrystallizationList = null;

	private Condition _condition;
	private IItemHandler _handler = IItemHandler.NULL;

	private boolean _bound; // Для скованных предметов = true
	 ...
	 * <LI>type</LI> <LI>_itemId</LI> <LI>_name</LI> <LI>_type1 & _type2</LI> <LI>_weight</LI> <LI>_crystallizable</LI> <LI>_stackable</LI> <LI>
	 * _materialType & _crystalType & _crystlaCount</LI> <LI>_durability</LI> <LI>_bodypart</LI> <LI>_referencePrice</LI> <LI>_sellable</LI>
	 *
	 * @param set
	...
	protected ItemTemplate(final StatsSet set)
	{
		...
		_crystalType = set.getEnum("crystal_type", Grade.class, Grade.NONE); // default
		// to
		// none-grade
		...
		_bound = set.getBool("bound", false);
	...
	/**
	 * Возвращает готовую для отображения в html строку вида <img src=icon.иконка width=32 height=32>
	 */
	...
	public void addExCrystallization(final CrystallizationItem item)
	{
		if(_exCrystallizationList == null)
		{
			//_exCrystallizationList = new ArrayList<>();
			_exCrystallizationList = new ArrayList<CrystallizationItem>();
		}

		_exCrystallizationList.add(item);
	}

	public List<CrystallizationItem> getExCrystallizationList()
	{
		return (_exCrystallizationList == null) ? Collections.<CrystallizationItem>emptyList() : _exCrystallizationList;
	}

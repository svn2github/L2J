...
public enum ExItemType
{
	...
	BIG_MAGIC_BLUNT_WEAPON(0), // 12 (WeaponType.BIGBLUNT) is_magic_weapon =
	// true
	...
	// Accessories
	RING(2), // 29 getBodyPart == ItemTemplate.SLOT_R_FINGER |
	// ItemTemplate.SLOT_L_FINGER
	EARRING(2), // 30 getBodyPart == ItemTemplate.SLOT_R_EAR |
	// ItemTemplate.SLOT_L_EAR
	NECKLACE(2), // 31 getBodyPart == ItemTemplate.SLOT_NECK
	BELT(2), // 32 getBodyPart == ItemTemplate.SLOT_BELT
	BRACELET(2), // 33 getBodyPart == ItemTemplate.SLOT_R_BRACELET ||
	// ItemTemplate.SLOT_L_BRACELET
	...
	SCROLL_OTHER(3), // 38 EtcItemType.SCROLL &&
	// SoulCrystalHolder.getInstance.getCrystal(int itemId) ==
	// null
	SOULSHOT(3), // 39 ItemTemplate.SOULSHOT_IDS.contains(itemId)
	SPIRITSHOT(3), // 40 ItemTemplate.SPIRITSHOT_IDS.contains(itemId) ||
	// ItemTemplate.BLESSED_SPIRITSHOT_IDS.contains(itemId)
	...
	// Etc
	CRYSTAL(5), // 44 itemId == ItemTemplate.CRYSTAL_NONE || itemId == CRYSTAL_D
	// = 1458 ...
	RECIPE(5), // 45 EtcItemType.RECIPE
	CRAFTING_MAIN_INGRIDIENTS(5), // 46 EtcItemType.MATERIAL в эту категорию
	// должны попадать только основные материалы,
	// необходимые для создания вещи.
	LIFE_STONE(5), // 47 ItemFunctions.isLifeStone(itemId) ||
	// ItemFunctions.isAccessoryLifeStone(itemId)
	SOUL_CRYSTAL(5), // 48 SoulCrystalHolder.getInstance.getCrystal(int itemId)
	// != null
	ATTRIBUTE_STONE(5), // 49 ItemTemplate.ATTRIBUTE_STONES.contains(itemId)
	WEAPON_ENCHANT_STONE(5), // 50
	// ItemFunctions.CATALYST_WEAPON.contains(itemId)
	...
	OTHER_ITEMS(5); // 59 Сюда попадает всё остальное, что не подошло ни под
	// одну категорию

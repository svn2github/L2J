...
import l2next.gameserver.model.base.BaseStats;
import l2next.gameserver.model.base.ClassId;
import l2next.gameserver.model.base.Element;
import l2next.gameserver.model.base.SkillTrait;
import l2next.gameserver.model.entity.events.GlobalEvent;
import l2next.gameserver.model.instances.ChestInstance;
...
import l2next.gameserver.network.serverpackets.FlyToLocation.FlyType;
...
import l2next.gameserver.skills.skillclasses.AIeffects;
import l2next.gameserver.skills.skillclasses.Aggression;
import l2next.gameserver.skills.skillclasses.Balance;
import l2next.gameserver.skills.skillclasses.BeastFeed;
import l2next.gameserver.skills.skillclasses.BuffCharger;
import l2next.gameserver.skills.skillclasses.CPDam;
import l2next.gameserver.skills.skillclasses.Call;
import l2next.gameserver.skills.skillclasses.ChainHeal;
import l2next.gameserver.skills.skillclasses.Charge;
import l2next.gameserver.skills.skillclasses.ChargeSoul;
import l2next.gameserver.skills.skillclasses.ClanGate;
import l2next.gameserver.skills.skillclasses.CombatPointHeal;
import l2next.gameserver.skills.skillclasses.Continuous;
import l2next.gameserver.skills.skillclasses.Craft;
import l2next.gameserver.skills.skillclasses.DeathPenalty;
import l2next.gameserver.skills.skillclasses.Decoy;
import l2next.gameserver.skills.skillclasses.Default;
import l2next.gameserver.skills.skillclasses.DefuseTrap;
import l2next.gameserver.skills.skillclasses.DeleteHate;
import l2next.gameserver.skills.skillclasses.DeleteHateOfMe;
import l2next.gameserver.skills.skillclasses.DestroySummon;
import l2next.gameserver.skills.skillclasses.DetectTrap;
import l2next.gameserver.skills.skillclasses.Disablers;
import l2next.gameserver.skills.skillclasses.Drain;
import l2next.gameserver.skills.skillclasses.DrainSoul;
import l2next.gameserver.skills.skillclasses.EMDam;
import l2next.gameserver.skills.skillclasses.EXPHeal;
import l2next.gameserver.skills.skillclasses.EffectsFromSkills;
import l2next.gameserver.skills.skillclasses.EnergyReplenish;
import l2next.gameserver.skills.skillclasses.ExtractStone;
import l2next.gameserver.skills.skillclasses.FishingSkill;
import l2next.gameserver.skills.skillclasses.Harvesting;
import l2next.gameserver.skills.skillclasses.Heal;
import l2next.gameserver.skills.skillclasses.HealHpCp;
import l2next.gameserver.skills.skillclasses.HealPercent;
import l2next.gameserver.skills.skillclasses.HealWithCp;
import l2next.gameserver.skills.skillclasses.KamaelWeaponExchange;
import l2next.gameserver.skills.skillclasses.KnowItem;
import l2next.gameserver.skills.skillclasses.LethalShot;
import l2next.gameserver.skills.skillclasses.MDam;
import l2next.gameserver.skills.skillclasses.ManaDam;
import l2next.gameserver.skills.skillclasses.ManaHeal;
import l2next.gameserver.skills.skillclasses.ManaHealPercent;
import l2next.gameserver.skills.skillclasses.NegateEffects;
import l2next.gameserver.skills.skillclasses.NegateStats;
import l2next.gameserver.skills.skillclasses.PDam;
import l2next.gameserver.skills.skillclasses.PcBangPointsAdd;
import l2next.gameserver.skills.skillclasses.PetSummon;
import l2next.gameserver.skills.skillclasses.Recall;
import l2next.gameserver.skills.skillclasses.ReelingPumping;
import l2next.gameserver.skills.skillclasses.Refill;
import l2next.gameserver.skills.skillclasses.Replace;
import l2next.gameserver.skills.skillclasses.Resurrect;
import l2next.gameserver.skills.skillclasses.Ride;
import l2next.gameserver.skills.skillclasses.SPHeal;
import l2next.gameserver.skills.skillclasses.ShiftAggression;
import l2next.gameserver.skills.skillclasses.Sowing;
import l2next.gameserver.skills.skillclasses.Spoil;
import l2next.gameserver.skills.skillclasses.StealBuff;
import l2next.gameserver.skills.skillclasses.Subjob;
import l2next.gameserver.skills.skillclasses.SummonItem;
import l2next.gameserver.skills.skillclasses.SummonMentor;
import l2next.gameserver.skills.skillclasses.SummonSiegeFlag;
import l2next.gameserver.skills.skillclasses.Sweep;
import l2next.gameserver.skills.skillclasses.SweepGoD;
import l2next.gameserver.skills.skillclasses.TakeCastle;
import l2next.gameserver.skills.skillclasses.TakeFlag;
import l2next.gameserver.skills.skillclasses.TakeFortress;
import l2next.gameserver.skills.skillclasses.TameControl;
import l2next.gameserver.skills.skillclasses.Teleport;
import l2next.gameserver.skills.skillclasses.TeleportNpc;
import l2next.gameserver.skills.skillclasses.Toggle;
import l2next.gameserver.skills.skillclasses.Transformation;
import l2next.gameserver.skills.skillclasses.Unlock;
import l2next.gameserver.skills.skillclasses.VitalityHeal;
...
public abstract class Skill extends StatTemplate implements Cloneable
{
	...
	public static enum SkillType
	{
		...
		EFFECT(l2next.gameserver.skills.skillclasses.Effect.class),
		...
		HARDCODED(l2next.gameserver.skills.skillclasses.Effect.class),
		...
		RESTORE_ITEM(l2next.gameserver.skills.skillclasses.Effect.class),
	...
	protected final int _referenceItemMpConsume; // количество потребляемой мп
	// талисмана
	...
	public static final int SKILL_DUAL_CAST = 11068; // Двойное Колдовство
	public static final int SKILL_SIGEL_FORCE = 1928; //Сила Сигеля
	public static final int SKILL_AERO_FORCE = 1940; //Сила Альгизы
	...
	protected int _bound_item;
	protected int _blessed_item;
	protected int _normal_item;
	protected int _unidentified_item;
	...
	public SystemMsg checkTarget(Creature activeChar, Creature target, Creature aimingTarget, boolean forceUse, boolean first)
	{
		...
		if(target == activeChar && isNotTargetAoE() || activeChar.getPets().contains(target) && _targetType == SkillTargetType.TARGET_PET_AURA)
		{
			return null;
		}
		...
		// Для различных бутылок, и для скилла кормления, дальнейшие проверки не
		// нужны
		...
		if(player != null)
		{
			// Запрет на атаку мирных NPC в осадной зоне на TW. Иначе таким
			// способом набивают очки.
			// if(player.getTerritorySiege() > -1 && target.isNpc() && !(target
			// instanceof L2TerritoryFlagInstance) && !(target.getAI()
			// instanceof DefaultAI) && player.isInZone(ZoneType.Siege))
			...
			if(pcTarget != null)
			{
				...
				if(pcTarget.isInOlympiadMode() && (!player.isInOlympiadMode() || player.getOlympiadGame() != pcTarget.getOlympiadGame())) // На
				// всякий
				// случай
				{
					return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				...
				if(isOffensive())
				{
					...
					if(player.isInOlympiadMode() && player.isOlympiadCompStart() && player.getOlympiadSide() == pcTarget.getOlympiadSide() && !forceUse) // Свою
					// команду
					// атаковать
					// нельзя
					{
					...
					if(activeChar.isInZoneBattle())
					{
						...
						return null; // Остальные условия на аренах и на
						// олимпиаде проверять не требуется
					}

					// Только враг и только если он еше не проиграл.
					/*
                          * Duel duel1 = player.getDuel(); Duel duel2 = pcTarget.getDuel(); if(player != pcTarget && duel1 != null && duel1 == duel2) {
                          * if(duel1.getTeamForPlayer(pcTarget) == duel1.getTeamForPlayer(player)) return SystemMsg.INVALID_TARGET;
                          * if(duel1.getDuelState(player.getStoredId()) != Duel.DuelState.Fighting) return SystemMsg.INVALID_TARGET;
                          * if(duel1.getDuelState(pcTarget.getStoredId()) != Duel.DuelState.Fighting) return SystemMsg.INVALID_TARGET; return null; }
                          */
					...
					if(isPvpSkill() || !forceUse || isAoE())
					{
                        ...
                        /*
                               ...
                               */
						// DS: атакующие скиллы не игнорируют командный канал и
						// альянс
                        /*
                               * if(player.isInParty() && player.getParty().getCommandChannel() != null && pcTarget.isInParty() &&
                               * pcTarget.getParty().getCommandChannel() != null && player.getParty().getCommandChannel() ==
                               * pcTarget.getParty().getCommandChannel()) return SystemMsg.INVALID_TARGET; if(player.getClan() != null &&
                               * player.getClan().getAlliance() != null && pcTarget.getClan() != null && pcTarget.getClan().getAlliance() != null &&
                               * player.getClan().getAlliance() == pcTarget.getClan().getAlliance()) return SystemMsg.INVALID_TARGET;
                               */
					}
                    ...
                    /*
                          * if(!forceUse && player.getPvpFlag() == 0 && pcTarget.getPvpFlag() != 0 && aimingTarget != target) return
				...
				if(player.isInOlympiadMode() && !forceUse && player.getOlympiadSide() != pcTarget.getOlympiadSide()) // Чужой
				// команде
				// помогать
				// нельзя
				{
				...
				// TODO [VISTALL] что за?
				// if(player.getTeam() > 0 && player.isChecksForTeam() &&
				// pcTarget.getTeam() > 0 && pcTarget.isChecksForTeam() &&
				// player.getTeam() != pcTarget.getTeam()) // Чужой команде
				// помогать
				// нельзя
				// return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				...
				// DS: на оффе можно использовать неатакующие скиллы из мирной
				// зоны в поле.
                /*
                ...
                /*
                     * if(player.getDuel() != null && pcTarget.getDuel() != player.getDuel()) return SystemMsg.INVALID_TARGET; if(player != pcTarget &&
                     * player.getDuel() != null && pcTarget.getDuel() != null && pcTarget.getDuel() == pcTarget.getDuel()) return
                     * SystemMsg.INVALID_TARGET;
                     */
	...
	public final Creature getAimingTarget(Creature activeChar, GameObject obj)
	{
		...
		switch(_targetType)
		{
			...
			case TARGET_ENEMY_PET:
				if(target == null || activeChar.getPets().contains(target) || !target.isPet())
				{
					return null;
				}
				return target;
			case TARGET_ENEMY_SUMMON:
				if(target == null || activeChar.getPets().contains(target) || !target.isSummon())
				{
					return null;
				}
				return target;
			case TARGET_ENEMY_SERVITOR:
				if(target == null || activeChar.getPets().contains(target) || !(target instanceof Summon))
				{
					return null;
				}
				return target;
			...
			case TARGET_MENTEE:
				// return target != null && target.isDead() == _isCorpse &&
				// target != activeChar && (!_isUndeadOnly || target.isUndead())
				// ? target : null;
			case TARGET_ONE:
			...
			case TARGET_CORPSE:
				...
				if(target.isSummon() && !activeChar.getPets().contains(target)) // использовать
				// собственного
				// мертвого
				// самона
				// нельзя
	...
	{
		GArray<Creature> targets;
		if(oneTarget())
		{
			targets = new GArray<Creature>(1);
			targets.add(aimingTarget);
			return targets;
		}
		else
		{
			targets = new GArray<Creature>();
		}
		...
		switch(_targetType)
		{
			...
			case TARGET_PET_AURA:
				if(activeChar.getFirstPet() == null)
				{
					break;
				}
				addTargetsToList(targets, activeChar.getFirstPet(), activeChar, forceUse);
				break;
			case TARGET_SUMMON_AURA:
				//Collection<Summon> servitors = activeChar.getPlayer().getPets();
				...
				if(activeChar.isMonster() || activeChar.isSiegeGuard())
				{
					...
					for(Creature c : World.getAroundCharacters(activeChar, _skillRadius, 600))
					{
						if(!c.isDead() && (c.isMonster() || c.isSiegeGuard()) /*
																				 * && ( ( L2MonsterInstance ) c ) . getFactionId ( ) . equals ( mob .
																				 * getFactionId ( ) )
																				 */)
						{
							targets.add(c);
	...
	private void addTargetAndPetToList(GArray<Creature> targets, Player actor, Player target)
	{
		if((actor == target || actor.isInRange(target, _skillRadius)) && target.isDead() == _isCorpse)
		{
			targets.add(target);
		}
		for(Summon pet : target.getPets())
		{
	...
	private void addTargetsToList(GArray<Creature> targets, Creature aimingTarget, Creature activeChar, boolean forceUse)
	{
		int count = 0;
		Polygon terr = null;
		...
		ThreadPoolManager.getInstance().execute(new RunnableImpl()
		{
			...
			public void runImpl()
			{
				...
				for(EffectTemplate et : getEffectTemplates())
				{
					...
					// Кастер в качестве цели также если скилл был отражен и
					// эффект отражабелен
					...
					{
						for(Summon summon : character.getPlayer().getPets())
						{
					...
					for(Creature target : targets)
					{
						...
						// effector.sendPacket(new
						// SystemMessage(SystemMessage.C1_HAS_RESISTED_YOUR_S2).addString(effected.getName()).addSkillName(_displayId,
						// _displayLevel));
						{
							continue;
						}
						...
						// effector.sendPacket(new
						// SystemMessage(SystemMessage.C1_WEAKLY_RESISTED_C2S_MAGIC).addName(effected).addName(effector));
						{
							continue;
						}
						...
						if(et._stackOrder == -1)
						{
							if(!et._stackTypes.equals(EffectTemplate.NO_STACK))
							{
								for(Effect e : target.getEffectList().getAllEffects())
									if(e.getStackType().equalsIgnoreCase(et._stackTypes))
										continue loop;
							}else if(target.getEffectList().getEffectsBySkillId(getId()) != null)
								continue;
							
							
							if(!et._stackTypes1.equals(EffectTemplate.NO_STACK))
							{
								for(Effect e : target.getEffectList().getAllEffects())
									if(e.getStackType().equalsIgnoreCase(et._stackTypes1))
										continue loop;
							}else if(target.getEffectList().getEffectsBySkillId(getId()) != null)
								continue;
							...
							if(!et._stackTypes2.equals(EffectTemplate.NO_STACK))
							{
								for(Effect e : target.getEffectList().getAllEffects())
									if(e.getStackType().equalsIgnoreCase(et._stackTypes2))
										continue loop;
							}else if(target.getEffectList().getEffectsBySkillId(getId()) != null)
								continue;
							...
							if(!et._stackTypes3.equals(EffectTemplate.NO_STACK))
							{
								for(Effect e : target.getEffectList().getAllEffects())
									if(e.getStackType().equalsIgnoreCase(et._stackTypes3))
										continue loop;
							}
							else if(target.getEffectList().getEffectsBySkillId(getId()) != null)
								continue;
						...
						if(success) // больше это значение не используется,
						// поэтому заюзываем его для
						// ConditionFirstEffectSuccess
						{
						...
						if(e != null)
						{
							...
							if(e.isOneTime())
							{
								// Эффекты однократного действия не шедулятся, а
								// применяются немедленно
								// Как правило это побочные эффекты для скиллов
							...
							else
							{
								...
								// Check for skill mastery duration time
								// increase
								if(skillMastery)
								{
									if(count > 1)
	...
	public Skill getSecondAddedSkill()
	{
		if(_addedSkills.length == 0)
		{
			return null;
		}
		return _addedSkills[1].getSkill();
	}
	...
	/**
	 * @return Возвращает ид предмета(талисмана) ману которого надо использовать
	 */
	...
	/**
	 * @return Возвращает используемое для каста количество маны предмета(талисмана)
	 */
	...
	public abstract void useSkill(Creature activeChar, GArray<Creature> targets);

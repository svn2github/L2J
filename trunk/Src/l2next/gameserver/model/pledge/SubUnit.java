...
import l2next.gameserver.network.serverpackets.ExSubPledgeSkillAdd;
...
public class SubUnit
{
	...
	public void removeUnitMember(int objectId)
	{
		...
		{
			setLeader(null, true); // clan leader has to assign another one, via
		}
		// villagemaster
		...
		try
		{
			...
			statement = con.prepareStatement(//
				"SELECT `c`.`char_name` AS `char_name`," + //
					...
					"`c`.`sex` AS `sex` " + //
					"FROM `characters` `c` " + //
					"LEFT JOIN `character_subclasses` `s` ON (`s`.`char_obj_id` = `c`.`obj_Id` AND `s`.`type` = '0') " + //

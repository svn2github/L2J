...
import l2next.gameserver.instancemanager.DimensionalRiftManager.DimensionalRiftRoom;
...
public class DimensionalRift extends Reflection
{
	...
	public DimensionalRift(Party party, int type, int room)
	{
		super();
		onCreate();
		startCollapseTimer(7200000); // 120 минут таймер, для защиты от утечек
		// памяти
		setName("Dimensional Rift");
		if(this instanceof DelusionChamber)
		{
			InstantZone iz = InstantZoneHolder.getInstance().getInstantZone(type + 120); // Для
			// равенства
			// типа
			// комнаты
			// и
	...
	protected void teleportToNextRoom()
	{
		...
		/*
		 * if(jumps_current < getMaxJumps()) size--; // комната босса может быть только последней
		 */

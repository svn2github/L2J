...
public class March8Event extends GlobalEvent
{
	...
	public void startEvent()
	{
		...
		Announcements.getInstance().announceToAll("MyTest startEvent");
	}
	...
	public void stopEvent()
	{
		...
		Announcements.getInstance().announceToAll("MyTest stopEvent");
	}

	@Override
	public void reCalcNextTime(boolean onInit)
	{
		clearActions(); // удаляем все дейсвтия которые остались(вообще то
		// ненужно, ибо они должны были все выполнится)

		// вызывается при старте, создаем дату, если она уже неправильная
		// добаляем 1 год
		if(onInit)
		{

...
import l2next.gameserver.model.entity.events.objects.DuelSnapshotObject;
import l2next.gameserver.model.party.Party;
import l2next.gameserver.network.serverpackets.ExDuelAskStart;
import l2next.gameserver.network.serverpackets.ExDuelEnd;
import l2next.gameserver.network.serverpackets.ExDuelReady;
...
public class PartyVsPartyDuelEvent extends DuelEvent
{
	...
	public void onDie(Player player)
	{
		...
		for(Player $player : World.getAroundPlayers(player))
		{
			...
			{
				$player.getAI().notifyEvent(CtrlEvent.EVT_FORGET_OBJECT, player.getPets());
			}
		}
		player.sendChanges();

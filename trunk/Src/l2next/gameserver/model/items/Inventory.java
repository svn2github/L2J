...
import l2next.gameserver.model.items.listeners.StatsListener;
...
public abstract class Inventory extends ItemContainer
{
	...
	private int findSlot()
	{
		...
		for(slot = 0; slot < _items.size(); slot++)
		{
			...
			{
				item = _items.get(i);
				if(item.isEquipped() || item.getTemplate().isQuest()) // игнорируем
				// надетое
				// и
				// квестовые
				// вещи
	...
	private void unEquipItemInBodySlot(int bodySlot, ItemInstance item)
	{
		...
		switch(bodySlot)
		{
			...
			case ItemTemplate.SLOT_HAIRALL:
				setPaperdollItem(PAPERDOLL_DHAIR, null); // This should be the
				// same as in DHAIR
				pdollSlot = PAPERDOLL_HAIR;
				break;

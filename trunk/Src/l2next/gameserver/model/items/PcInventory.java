...
import l2next.gameserver.model.items.listeners.AccessoryListener;
import l2next.gameserver.model.items.listeners.ArmorSetListener;
import l2next.gameserver.model.items.listeners.BowListener;
import l2next.gameserver.model.items.listeners.ItemAugmentationListener;
import l2next.gameserver.model.items.listeners.ItemEnchantOptionsListener;
import l2next.gameserver.model.items.listeners.ItemSkillsListener;
import l2next.gameserver.network.serverpackets.*;
import l2next.gameserver.taskmanager.DelayedItemsManager;
...
public class PcInventory extends Inventory
{
	...
	/**
	 * Функция для валидации вещей в инвентаре. Снимает все вещи, которые нельзя носить. Применяется при входе в игру, смене саба, захвате замка,
	 * выходе из клана.
	 */
	...
	public void restore()
	{
		...
		try
		{
			...
			// CollectionUtils.eqSort(_items,
			// ItemOrderComparator.getInstance());
			...
			for(ItemInstance item : items)
			{
				...
				if(item.getEquipSlot() >= PAPERDOLL_MAX)
				{
					...
					item.setLocData(0); // Немного некрасиво, но инвентарь еще
					// весь не загружен и свободный слот не
					// найти

...
import l2next.commons.dao.JdbcEntity;
...
import l2next.gameserver.network.serverpackets.DropItem;
import l2next.gameserver.network.serverpackets.L2GameServerPacket;
import l2next.gameserver.network.serverpackets.SpawnItem;
...
import l2next.gameserver.templates.item.ItemType;
...
public final class ItemInstance extends GameObject implements JdbcEntity
{
	...
	{
		VOID, INVENTORY, PAPERDOLL, PET_INVENTORY, PET_PAPERDOLL, WAREHOUSE, CLANWH, FREIGHT, // востановлен,
		// используется
		// в
		// Dimension
		// Manager
	...
	public boolean canBeAugmented(Player player, LifeStone.StoneGrade lsg)
	{
		if(!canBeEnchanted())
		{
			return false;
		...
		if(isAccessory())
		{
			return lsg == LifeStone.StoneGrade.ACCESSORY;
		}
		...
		if(isWeapon())
		{
			return (lsg != LifeStone.StoneGrade.ACCESSORY && lsg != LifeStone.StoneGrade.UNDERWEAR);
		}
	...
	public void dropToTheGround(Player lastAttacker, NpcInstance fromNpc)
	{
		...
		// Init the dropped L2ItemInstance and add it in the world as a visible
	...
	{
		// FIXME кажись дроппер у нас есть в итеме как переменная, ток проверить
		// время? [VISTALL]

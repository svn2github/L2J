...
import l2next.gameserver.network.serverpackets.BuyListSeed;
import l2next.gameserver.network.serverpackets.ExShowCropInfo;
import l2next.gameserver.network.serverpackets.ExShowManorDefaultInfo;
import l2next.gameserver.network.serverpackets.ExShowProcureCropDetail;
import l2next.gameserver.network.serverpackets.ExShowSeedInfo;
import l2next.gameserver.network.serverpackets.ExShowSellCropList;
...
public class ManorManagerInstance extends MerchantInstance
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -3364448278566901708L;
		...
		else
		{
			...
			else
			{
				if(CastleManorManager.getInstance().isDisabled())
				{
					...
					&& player.isClanLeader() // Player is clan leader of
					// clan (then he is the lord)
					&& getCastle() != null // Verification of castle
					&& getCastle().getOwnerId() == player.getClanId() // Player's
					// clan
					// owning
					// the
					// castle

...
import l2next.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
...
import l2next.gameserver.network.serverpackets.PledgeStatusChanged;
...
public final class VillageMasterInstance extends NpcInstance
{
	...
	public void setLeader(Player leader, String newLeader)
	{
		...
		if(member == null)
		{
			...
			// CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.S1IsNotMemberOfTheClan",
		...
		{
			leader.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.CannotAssignUnitLeader", leader));
			return;
		}
	...
	{
		player.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.ClanLeaderWillBeChangedFromS1ToS2", player).addString(clan.getLeaderName()).addString(newLeader.getName()));
		// TODO: В данной редакции смена лидера производится сразу же.
		// Надо подумать над реализацией смены кланлидера в запланированный день
		// недели.

		/*
		 * if(clan.getLevel() >= CastleSiegeManager.getSiegeClanMinLevel()) { if(clan.getLeader() != null) { L2Player oldLeaderPlayer =
		 * clan.getLeader().getPlayer(); if(oldLeaderPlayer != null) SiegeUtils.removeSiegeSkills(oldLeaderPlayer); } L2Player newLeaderPlayer =
		 * newLeader.getPlayer(); if(newLeaderPlayer != null) SiegeUtils.addSiegeSkills(newLeaderPlayer); }
		 */
	...
	public void createSubPledge(Player player, String clanName, int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if(pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if(subLeader == null)
			{
				player.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader", player));
				return;
			}
			else if(subLeader.getLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader", player));
				return;
			}
		}
	...
	public void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if(clan == null)
		{
			player.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.ClanDoesntExist", player));
			return;
		}
		...
		if(targetUnit == null)
		{
			player.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.SubUnitNotFound", player));
			return;
		}
		SubUnit mainUnit = clan.getSubUnit(Clan.SUBUNIT_MAIN_CLAN);
		...
		player.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.NewSubUnitLeaderHasBeenAssigned", player));
	}
	...
	public void levelUpClan(Player player)
	{
		...
		switch(clan.getLevel())
		{
			...
			case 5:
				// Upgrade to 6
				if(clan.getReputationScore() >= 5000)
				{
			...
			case 6:
				// Upgrade to 7
				if(clan.getReputationScore() >= 10000)
				{
			...
			case 7:
				// Upgrade to 8
				if(clan.getReputationScore() >= 20000)
				{
			...
			case 8:
				...
				if(clan.getReputationScore() >= 40000)
				{
			...
			case 10:
				// Upgrade to 11
				if(clan.getReputationScore() >= 75000 && clan.getCastle() > 0)
				{

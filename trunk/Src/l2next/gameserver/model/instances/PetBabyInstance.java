...
public final class PetBabyInstance extends PetInstance
{
		...
		try
		{
			...
			if(!owner.isDead() && !owner.isInvul() && !isCastingNow())
			{
				...
				if(!Config.ALT_PET_HEAL_BATTLE_ONLY || owner.isInCombat())
				{
					...
					// проверка речарджа
					if(skill == null && getNpcId() == PetDataTable.IMPROVED_BABY_KOOKABURRA_ID) // Речардж
					// только
					// у
					// Kookaburra
					// и
	...
	{
		if(ef == null || !ef.isInUse() || !EffectList.checkStackType(ef.getTemplate(), skill.getEffectTemplates()[0])) // такого
		// скилла
		// нет
		{
			return false;
		}
		if(ef.getStackOrder() < skill.getEffectTemplates()[0]._stackOrder) // старый
		// слабее
		{
			return false;
		}
		...
		if(ef.getNext() != null) // старый не слабее но уже кончается -
		// проверить рекурсией что там зашедулено
		{

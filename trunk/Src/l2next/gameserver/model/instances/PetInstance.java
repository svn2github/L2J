...
import l2next.gameserver.model.PetData;
...
public class PetInstance extends Summon
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -9115829728250435721L;
	...
	class FeedTask extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			else if(getCurrentFed() <= 0.10 * getMaxFed())
			{
				...
				owner.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2PetInstance.UnSummonHungryPet", owner));
				unSummon();
				return;
			}
	...
	public void addExpAndSp(long addToExp, long addToSp)
	{
		...
		if(old_level < _level)
		{
			owner.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2PetInstance.PetLevelUp", owner).addNumber(_level));
	...
	public long getMaxExp()
	{
		...
		{
			_log.error("getMaxExp() petNpcId: " + getNpcId() + "Exception: " + e);
			return 1;
		}
	}
	...
	{
		// В базе указаны параметры, уже домноженные на этот модификатор, для
		// удобства. Поэтому вычисляем и убираем его.
	...
	public void updateControlItem()
	{
		...
		owner.sendPacket(new ExAdenaInvenCount(owner));
	...
	{
		return 0;// TODO [VISTALL] objectId если бафы петов сохраняются
	}
}
...
public final class TamedBeastInstance extends FeedableBeastInstance
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -4009621201975741486L;
	...
	static
	{
		TAMED_DATA[0] = new AbstractMap.SimpleImmutableEntry<NpcString, int[]>(NpcString.RECKLESS_S1, new int[]{6671});
	...
	private static class CheckDuration extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			// I tried to avoid this as much as possible...but it seems I can't
			// avoid hardcoding
			// ids further, except by carrying an additional variable just for
			// these two lines...
			...
			// if the owner has enough food, call the item handler (use the food
			// and triffer all necessary actions)
			if(item != null && item.getCount() >= 1)
			{
				_tamedBeast.onReceiveFood();
				owner.getInventory().destroyItem(item, 1);
			}
			else // if the owner has no food, the beast immediately despawns,

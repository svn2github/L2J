...
public class FeedableBeastInstance extends MonsterInstance
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -2360683048059946184L;
	...
	private void spawnNext(Player player, int growthLevel, int food, int skill_id)
	{
		...
		// if this is finally a trained mob, then despawn any other trained mobs
		// that the player might have and initialize the Tamed Beast.
		...
		// if not trained, the newly spawned mob will automatically be agro
		// against its feeder (what happened to
		// "never bite the hand that feeds you" anyway?!)
		else
		{
			// spawn the new mob
			MonsterInstance nextNpc = spawn(nextNpcId, getX(), getY(), getZ());
			feedInfo.put(nextNpc.getObjectId(), player.getObjectId()); // register
			// the
			// player
			// in the
			// feedinfo
	...
	public void onSkillUse(Player player, int skillId)
	{
		...
		{
			// do nothing if this mob doesn't eat the specified food (food gets
			// consumed but has no effect).
			...
			if(growthLevel > 0)
			// check if this is the same player as the one who raised it
			// from growth 0.
			// if no, then do not allow a chance to raise the pet (food gets
			// consumed but has no effect).
			{
			...
			// Polymorph the mob, with a certain chance, given its current
			// growth level

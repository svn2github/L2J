...
public class MonsterInstance extends NpcInstance
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -3898620551123015640L;
	...
	protected void onReduceCurrentHp(double damage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp)
	{
		if(skill != null && skill.isOverhit())
		{
			...
			// Ex: mob had 10 HP left, over-hit skill did 50 damage total,
			// over-hit damage is 40
	...
	public void calculateRewards(Creature lastAttacker)
	{
		...
		if (getTemplate().rewardCRp > 0 && topDamager.getPlayer() != null)
        {
            final Clan clan = topDamager.getPlayer().getClan();
            if (clan != null && clan.getLevel() >= 0)
            {
                clan.incReputation(getTemplate().rewardCRp, true, "monster reward");
            }
        }
	...
	public void rollRewards(Map.Entry<RewardType, RewardList> entry, final Creature lastAttacker, Creature topDamager)
	{
		...
		switch(type)
		{
			...
			default:
				...
				{
					// Если в моба посеяно семя, причем не альтернативное - не
					// давать никакого дропа, кроме адены.

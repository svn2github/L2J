...
{
	public static final int BROADCAST_CHAR_INFO = 1 << 0; // Требуется обновить
	// состояние персонажа
	// у окружающих
	public static final int SEND_CHAR_INFO = 1 << 1; // Требуется обновить
	// состояние только самому
	// персонажу
	public static final int SEND_STATUS_INFO = 1 << 2; // Требуется обновить
	// статус HP/MP/CP
	...
	protected GArray<AbnormalEffect> _abnormalEffects;
	...
	protected GArray<AbnormalEffect> set(int flag, GArray<AbnormalEffect> oldValue, GArray<AbnormalEffect> newValue)
	{
		if(oldValue == null)
		{
			oldValue = _activeChar.oldValue;
		}
	...
	protected void refreshStats()
	{
		...
		_abnormalEffects = set(BROADCAST_CHAR_INFO, _abnormalEffects, _activeChar.getAbnormalEffects());

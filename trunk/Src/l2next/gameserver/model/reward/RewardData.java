...
public class RewardData implements Cloneable
{
	...
	public RewardData(int itemId)
	{
		...
		if(_item.isArrow() // стрелы не рейтуются
			|| Config.NO_RATE_EQUIPMENT && _item.isEquipment() // отключаемая
			// рейтовка
			// эквипа
			|| Config.NO_RATE_KEY_MATERIAL && _item.isKeyMatherial() // отключаемая
			// рейтовка
			// ключевых
			// материалов
			|| Config.NO_RATE_RECIPES && _item.isRecipe() // отключаемая
			// рейтовка
			// рецептов
			|| ArrayUtils.contains(Config.NO_RATE_ITEMS, itemId)) // индивидаульная
		// отключаемая
		// рейтовка
		// для
		// списка

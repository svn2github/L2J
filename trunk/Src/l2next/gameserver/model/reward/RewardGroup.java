package l2next.gameserver.model.reward;
...
public class RewardGroup implements Cloneable
{
	...
	private boolean _isAdena = false; // Шанс фиксирован, растет только
	// количество
	...
	public List<RewardItem> roll(RewardType type, Player player, double mod, boolean isRaid, boolean isSiegeGuard)
	{
		switch(type)
		{
			...
			case SWEEP:
				return rollItems(mod, Config.RATE_DROP_SPOIL + player.getVitality().getDropMod(), player.getRateSpoil());
			case RATED_GROUPED:
				if(_isAdena)
				{
					return rollAdena(mod, Config.RATE_DROP_ADENA + player.getVitality().getDropMod(), player.getRateAdena());
				}

				if(isRaid)
				{
					return rollItems(mod, Config.RATE_DROP_RAIDBOSS + player.getVitality().getDropMod(), 1.0);
				}

				if(isSiegeGuard)
				{
					return rollItems(mod, Config.RATE_DROP_SIEGE_GUARD + player.getVitality().getDropMod(), 1.0);
				}

				return rollItems(mod, Config.RATE_DROP_ITEMS + player.getVitality().getDropMod(), player.getRateItems());
			default:

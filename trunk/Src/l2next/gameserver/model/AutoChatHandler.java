...
import l2next.gameserver.network.serverpackets.NpcSayNative;
...
/**
 * @author ALF Урезаный AutoChatHandler предназначен ТОЛЬКО для фраз НПЦ (серым цветом) и только ГЛОБАЛЬНЫХ!
 */
public class AutoChatHandler implements OnSpawnListener
{
	...
	public class AutoChatInstance
	{
		...
		{
			GArray<NpcInstance> npcInsts = new GArray<NpcInstance>();
			...
			public synchronized void run()
			{
				try
				{
					...
					for(NpcInstance npc : GameObjectsStorage.getAllByNpcId(_npcId, true))
					{
						...
						GArray<Player> nearbyPlayers = World.getAroundPlayers(chatNpc, 1500, 200);

...
import l2next.gameserver.taskmanager.SpawnTaskManager;
...
public abstract class Spawner extends EventOwner implements Cloneable
{
	...
	protected NpcInstance initNpc0(NpcInstance mob, Location newLoc, boolean spawn, MultiValueSet<String> set)
	{
		...
		if(spawn)
		{
			...
			// Init other values of the L2NpcInstance (ex : from its
			// L2CharTemplate for INT, STR, DEX...) and add it in the world as a
			// visible object
			mob.spawnMe(newLoc);

			// Increase the current number of L2NpcInstance managed by this
			// L2Spawn
			_currentCount++;
		}
		else
		{
			...
			// Update the current number of SpawnTask in progress or stand by of
			// this L2Spawn
			_scheduledCount++;

...
{
	private static final long serialVersionUID = 828762795157099229L;
	/**
	...
	public boolean checkPvP(final Creature target, Skill skill)
	{
		...
		if(isDead() || target == null || player == null || target == this || target == player || player.getPets().contains(target) || player.getKarma() < 0)
		{
			return false;
		}
	...
	public boolean checkTarget(Creature target)
	{
		...
		// instanceof L2TerritoryFlagInstance) && !(target.getAI() instanceof
		// DefaultAI) && player.isInZone(ZoneType.Siege))
		// {
		// player.sendPacket(Msg.INVALID_TARGET);
		// return false;
		// }
		...
		{
			// Нельзя атаковать того, кто находится на арене, если ты сам не на
			// арене
			if(isInZoneBattle() != target.isInZoneBattle())
			{
				player.sendPacket(Msg.INVALID_TARGET);
			...
			// Если цель либо атакующий находится в мирной зоне - атаковать
			// нельзя
			if(isInZonePeace() || target.isInZonePeace())
			{
	...
	public void doAttack(Creature target)
	{
		...
		{
			player.sendMessage(new CustomMessage("l2next.gameserver.model.L2Playable.OutOfControl.ObserverNoAttack", player));
			return;
		}
	...
	public void callSkill(Skill skill, GArray<Creature> targets, boolean useActionSkills)
	{
		...
		if(useActionSkills && !skill.altUse() && !skill.getSkillType().equals(SkillType.BEAST_FEED))
		{
			for(Creature target : targets)
			{
				...
				// исключать баффы питомца на владельца
				else if(target.isPlayable() && !getPets().contains(target) && !((this.isSummon() || this.isPet()) && target == player))
				{
					int aggro = skill.getEffectPoint() != 0 ? skill.getEffectPoint() : Math.max(1, (int) skill.getPower());

					GArray<NpcInstance> npcs = World.getAroundNpc(target);
					for(NpcInstance npc : npcs)
					{
						if(npc.isDead() || !npc.isInRangeZ(this, 2000)) // FIXME
						// [G1ta0]
						// параметр
						// достойный
						// конфига
						...
						if(GeoEngine.canSeeTarget(npc, target, false)) // Моб
						// агрится
						// только
						// если
						// видит

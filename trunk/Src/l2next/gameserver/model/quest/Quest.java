package l2next.gameserver.model.quest;

import gnu.trove.map.hash.TIntObjectHashMap;
import gnu.trove.set.hash.TIntHashSet;
import l2next.commons.dbutils.DbUtils;
import l2next.commons.logging.LogUtils;
import l2next.commons.threading.RunnableImpl;
import l2next.commons.util.TroveUtils;
...
import l2next.gameserver.model.quest.startcondition.impl.ClassLevelCondition;
import l2next.gameserver.model.quest.startcondition.impl.PlayerLevelCondition;
import l2next.gameserver.model.quest.startcondition.impl.QuestCompletedCondition;
import l2next.gameserver.network.serverpackets.ExNpcQuestHtmlMessage;
...
public class Quest
{
	...
	private List<ICheckStartCondition> startConditionList = new ArrayList<ICheckStartCondition>();

	/**
	 * Этот метод для регистрации квестовых вещей, которые будут удалены при прекращении квеста, независимо от того, был он закончен или прерван.
	 * <strong>Добавлять сюда награды нельзя</strong>.
	 */
	 ...
	 * <U><I>Actions :</I></U><BR>
	 * <LI>Get ID state of the quest recorded in object qs</LI> <LI>Save in database the ID state (with or without the star) for the variable called
	...
	/**
	 * Добавляет нпц масив для слушателя при их убийстве, и обновлении пакетом {@link l2next.gameserver.network.serverpackets.ExQuestNpcLogList}
	 *
	 * @param cond
	 * @param varName
	 * @param killIds
	 ...
	 *
	 * @param classLevels
	 *            - список уровней классов, для которых доступен квест
	 */
	...
	public final int getDescrState(Player player, boolean isStartNpc)
	{
		...
		{ // TODO [Ragnarok], уточнить нужна ли здесь проверка на
			// isAvailableFor()
			state = 2;
		}
	...
	public final boolean notifyFirstTalk(NpcInstance npc, Player player)
	{
		...
		// if the quest returns text to display, display it. Otherwise, use the
		// default npc text.
	 ...
	 * <LI><U>"res" ends with string ".html" :</U> an HTML is opened in order to be shown in a dialog box</LI> <LI><U>"res" starts with tag "html"
	 * :</U> the message hold in "res" is shown in a dialog box</LI> <LI><U>"res" is null :</U> do not show any message</LI> <LI><U>"res" is empty
	 * string :</U> show default message</LI> <LI><U>otherwise :</U> the message hold in "res" is shown in chat box</LI>
	 *
	 * @param npc
	 * @param player
	 * @param res
	 ...
	 *
	 * @param player
	 *            - игрок, для которого проверяем условия квеста

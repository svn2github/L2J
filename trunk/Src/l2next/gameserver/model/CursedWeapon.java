...
import l2next.gameserver.network.serverpackets.ExRedSky;
...
public class CursedWeapon
{
	...
	public boolean reActivate()
	{
		if(getTimeLeft() <= 0)
		{
			if(getPlayerId() != 0) // to be sure, that cursed weapon will
			// deleted in right way
			{
	...
	{
		if(isDropped() || getPlayerId() != player.getObjectId()) // оружие уже
		// в руках
		// игрока или
		// новый игрок
		{
	...
	public void setPlayer(Player player)
	{
		...
		{
			setPlayerId(getPlayerId()); // для того что бы сохранить objId, но
		}
		// не искать игрока в хранилище
	}

...
public class Manor
{
	...
	public class SeedData
	{
		...
		private int _type1;
		private int _type2;
		private int _manorId; // id of manor (castle id) where seed can be
		// farmed
	...
	private SeedData parseList(String line)
	{
		...
		int manorId = Integer.parseInt(st.nextToken()); // id of manor, where
		// seed can be farmed
		int isAlt = Integer.parseInt(st.nextToken()); // alternative seed
		long limitSeeds = Math.round(Integer.parseInt(st.nextToken()) * Config.RATE_MANOR); // limit
		// for
		// seeds
		long limitCrops = Math.round(Integer.parseInt(st.nextToken()) * Config.RATE_MANOR); // limit
		// for
		// crops

package l2next.gameserver.security;

import l2next.commons.dbutils.DbUtils;
import l2next.gameserver.Base64;
import l2next.gameserver.Config;
import l2next.gameserver.database.DatabaseFactory;
import l2next.gameserver.network.GameClient;
import l2next.gameserver.network.serverpackets.Ex2ndPasswordAck;
import l2next.gameserver.network.serverpackets.Ex2ndPasswordCheck;
import l2next.gameserver.network.serverpackets.Ex2ndPasswordVerify;
...
public class SecondaryPasswordAuth
{
	...
	private boolean validatePassword(String password)
	{
		...
		/*
		 * for (int i = 0; i < password.length()-1; i++) { char curCh = password.charAt(i); char nxtCh = password.charAt(i+1);
		 * 
		 * if (curCh+1 == nxtCh) return false; else if (curCh-1 == nxtCh) return false; else if (curCh == nxtCh) return false; }
		 * 
		 * for (int i = 0; i < password.length()-2; i++) { String toChk = password.substring(i+1); StringBuffer chkEr = new
		 * StringBuffer(password.substring(i, i+2));
		 * 
		 * if (toChk.contains(chkEr)) return false; else if (toChk.contains(chkEr.reverse())) return false; }
		 */
		_wrongAttempts = 0;
		return true;
	}

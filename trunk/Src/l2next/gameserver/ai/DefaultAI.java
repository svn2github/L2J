...
import l2next.commons.collections.CollectionUtils;
import l2next.commons.collections.GArray;
import l2next.commons.lang.reference.HardReference;
import l2next.commons.math.random.RndSelector;
...
import l2next.gameserver.model.WorldRegion;
...
import l2next.gameserver.taskmanager.AiTaskManager;
...
public class DefaultAI extends CharacterAI
{
	...
	private boolean _isTargetable = false;
	...
	protected boolean thinkActive()
	{
		...
		if(now - _checkAggroTimestamp > Config.AGGRO_CHECK_INTERVAL)
		{
			...
			{
				GArray<Creature> chars = World.getAroundCharacters(actor);
	...
	{
		NpcInstance actor = getActor();
		if(target == null || target.isAlikeDead() || !actor.isInRangeZ(target, range) || target.isTargetable() == false)
		{
			return false;
		}
	...
	protected boolean tryMoveToTarget(Creature target, int range)
	{
		...
		if(_pathfindFails >= getMaxPathfindFails() && (System.currentTimeMillis() > getAttackTimeout() - getMaxAttackTimeout() + getTeleportTimeout()) && actor.isInRange(target, MAX_PURSUE_RANGE))
		{
			...
			if(!GeoEngine.canMoveToCoord(actor.getX(), actor.getY(), actor.getZ(), loc.x, loc.y, loc.z, actor.getGeoIndex())) // Для
			// подстраховки
			{
				loc = target.getLoc();
			}
		...
		switch(currentTask.type)
		{
			...
			case MOVE:
			{
				...
				if(!actor.moveToLocation(currentTask.loc, 0, currentTask.pathfind))
				{
					...
					// actor.broadcastPacketToOthers(new MagicSkillUse(actor,
					// actor, 2036, 1, 500, 600000));
					// ThreadPoolManager.getInstance().scheduleAi(new
					// Teleport(currentTask.loc), 500, false);
					return maybeNextTask(currentTask);
				}
			}
			break;
	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		// Обычно 1 хейт добавляется хозяину суммона, чтобы после смерти суммона
		// моб накинулся на хозяина.
	...
	protected Creature prepareTarget()
	{
		...
		GArray<Creature> hateList = actor.getAggroList().getHateList();
		Creature hated = null;
		for(Creature cha : hateList)
		{
			// Если у Монстра есть скилл "Searching Master" он должен атаковать
			// хозяина пета в первую очередь.
	...
	protected GArray<NpcInstance> activeFactionTargets()
	{
		NpcInstance actor = getActor();
		...
		GArray<NpcInstance> npcFriends = new GArray<NpcInstance>();
	...
	protected boolean defaultThinkBuff(int rateSelf, int rateFriends)
	{
		...
		// TODO сделать более разумный выбор баффа, сначала выбирать подходящие
		// а потом уже рандомно 1 из них

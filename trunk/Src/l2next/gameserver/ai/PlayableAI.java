...
import l2next.gameserver.model.Skill.NextAction;
...
import static l2next.gameserver.ai.CtrlIntention.AI_INTENTION_ATTACK;
import static l2next.gameserver.ai.CtrlIntention.AI_INTENTION_CAST;
import static l2next.gameserver.ai.CtrlIntention.AI_INTENTION_FOLLOW;
import static l2next.gameserver.ai.CtrlIntention.AI_INTENTION_INTERACT;
import static l2next.gameserver.ai.CtrlIntention.AI_INTENTION_PICK_UP;
...
public class PlayableAI extends CharacterAI
{
		...
		try
		{
			...
			switch(getIntention())
			{
				...
				case AI_INTENTION_COUPLE_ACTION:
					...
					break;
				default:
					_log.warn("Intention is not found " + getIntention());
					break;
			}
		}
		catch(Exception e)
	...
	protected class ThinkFollow extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			{
				// Если пет прекратил преследование, меняем статус, чтобы не
				// пришлось щелкать на кнопку следования 2 раза.
			...
			if(player == null || player.isLogoutStarted() || (actor.isPet() || actor.isSummon()) && !player.hasPet(actor.getObjectId()))
			{
				setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
				return;
			}
	...
	protected void thinkCast(boolean checkRange)
	{
		...
		if(!checkRange)
		{
			// Если скилл имеет следующее действие, назначим это действие после
			// окончания действия скилла
	...
	public void lockTarget(Creature target)
	{
		...
		else if(actor.getAggressionTarget() == null)
		{
			...
			clearNextAction();
			// DS: агрессия только перекидывает видимую цель, но не обрывает
			// текущую атаку/каст
			/*
			 * if (getIntention() == CtrlIntention.AI_INTENTION_ATTACK) setAttackTarget(target); switch(getIntention()) { case AI_INTENTION_ATTACK:
			 * setAttackTarget(target); break; case AI_INTENTION_CAST: L2Skill skill = actor.getCastingSkill(); if(skill == null) skill = _skill;
			 * if(skill != null && !skill.isUsingWhileCasting()) switch(skill.getTargetType()) { case TARGET_ONE: case TARGET_AREA: case
			 * TARGET_MULTIFACE: case TARGET_TUNNEL: setAttackTarget(target); actor.setCastingTarget(target); break; } break; }
			 */
	...
	public void Cast(Skill skill, Creature target, boolean forceUse, boolean dontMove)
	{
		...
		// то он может использоваться во время каста других скиллов, или во
		// время атаки, или на бегу.

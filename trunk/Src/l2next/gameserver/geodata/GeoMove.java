...
public class GeoMove
{
	...
	private static List<List<Location>> getNodePath(List<Location> path, int geoIndex)
	{
		...
		for(int i = 1; i < size; i++)
		{
			...
			List<Location> moveList = GeoEngine.MoveList(p1.x, p1.y, p1.z, p2.x, p2.y, geoIndex, true); // onlyFullPath
			// =
			// true
			// -
			// проверяем
			...
			// до
			// конца
			if(moveList == null) // если хотя-бы через один из участков нельзя
			// пройти, забраковываем весь путь
			{
				return Collections.emptyList();
			}
			if(!moveList.isEmpty()) // это может случиться только если 2
			// одинаковых точки подряд
			{
				result.add(moveList);
			}
		}
	...
	private static void pathClean(List<Location> path, int geoIndex)
	{
		...
		if(size > 2)
		{
			for(int i = 2; i < size; i++)
			{
				...
				Location p2 = path.get(i - 1); // точка в середине, кандидат на
				// вышибание
				Location p1 = path.get(i - 2); // точка начала движения
				if(p1.equals(p2) || p3.equals(p2) || IsPointInLine(p1, p2, p3)) // если
				// вторая
				// точка
				// совпадает
				// с
				...
				// нужна
				{
					...
					i = Math.max(2, i - 2); // сдвигаемся назад, FIXME: может я
					// тут не совсем прав
				}
			}
		}
	...
	private static boolean IsPointInLine(Location p1, Location p2, Location p3)
	{
		...
		// Это работает потому, что сравниваем мы соседние точки (расстояния
		// между ними равны, важен только знак).

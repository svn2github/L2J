...
import l2next.commons.executor.ParallelExecutor;
import l2next.commons.geometry.Polygon;
import l2next.gameserver.Config;
import l2next.gameserver.geodata.GeoOptimizer.BlockLink;
...
public class GeoEngine
{
	...
	public static int MAX_LAYERS = 1; // меньше 1 быть не должно, что бы
	// создавались временные массивы как
	// минимум short[2]
	...
	{
		return NgetType(x - World.MAP_MIN_X >> 4, y - World.MAP_MIN_Y >> 4, refIndex);
	}
	...
	public static int getHeight(int x, int y, int z, int refIndex)
	{
		...
		return NgetHeight(x - World.MAP_MIN_X >> 4, y - World.MAP_MIN_Y >> 4, z, refIndex);
	}
	...
	{
		return NgetNSWE(x - World.MAP_MIN_X >> 4, y - World.MAP_MIN_Y >> 4, z, refIndex);
	}
	...
	{
		return MoveCheckForAI(loc1.x - World.MAP_MIN_X >> 4, loc1.y - World.MAP_MIN_Y >> 4, loc1.z, loc2.x - World.MAP_MIN_X >> 4, loc2.y - World.MAP_MIN_Y >> 4, refIndex);
	}

	public static Location moveCheckInAir(int x, int y, int z, int tx, int ty, int tz, double ColRadius, int refIndex)
	{
		return moveCheckInAir(x, y, z, tx, ty, tz, (float) ColRadius, refIndex);
	}
	...
	public static boolean canSeeTarget(GameObject actor, GameObject target, boolean air)
	{
		...
		return canSeeCoord(actor, target.getX(), target.getY(), target.getZ() + (int) target.getColHeight() + 64/*
																												 * , actor . isPlayer
																												 */, air);
	}
	...
	private static boolean NLOS_WATER(int x, int y, int z, int next_x, int next_y, int next_z, int refIndex)
	{
		...
		// Либо перед нами стена, либо над нами потолок. Ищем слой пониже, для
		// уточнения
	 ...
	 * проверка видимости
	 *
	 * @return возвращает последнюю точку которую видно (в формате геокоординат) в результате (Location) h является кодом, если >= 0 то успешно
	 *         достигли последней точки, если меньше то не последней
	 */
	public static Location canSee(int _x, int _y, int _z, int _tx, int _ty, int _tz, boolean air, int refIndex)
	{
		...
		for(int i = 0; i < steps; i++)
		{
			...
			} // либо есть преграждающая поверхность, либо нет снизу слоя и
			// значит это "пустота", то что за стеной или за колоной
			NGetLayers(curr_x, curr_y, curr_layers, refIndex);
			if(curr_layers[0] == 0)
			{
				result.set(_tx, _ty, _tz, 0);
			...
			} // либо есть преграда, либо нет снизу слоя и значит это "пустота",
			// то что за стеной или за колоной
			if(curr_x == i_next_x)
			{
			...
			else
			{
				...
				} // либо есть преграда, либо нет снизу слоя и значит это
				// "пустота", то что за стеной или за колоной
	...
	public static Location MoveCheck(int __x, int __y, int _z, int __tx, int __ty, boolean withCollision, boolean backwardMove, boolean returnPrev, int refIndex)
	{
		...
		// return new Location(__x, __y, _z);
		// log.info("move" + (backwardMove ? " back" : "") + (withCollision ?
		// " +collision" : "") + ": " + curr_x + " " + curr_y + " " + curr_z +
		// " / xyz: " + __x + " " + __y + " " + _z + " / to xy: " + __tx + " " +
		// __ty + " / geo xy: " + _x + " " + _y + " / geo to xy: " + _tx + " " +
		// _ty);
	...
	public static void NGetLayers(int geoX, int geoY, short[] result, int refIndex)
	{
		...
		switch(type)
		{
			...
			case BLOCKTYPE_MULTILEVEL:
				...
				return;
			default:
				log.info("GeoEngine: Unknown block type");
				return;
		}
	}
	...
	public static int NgetHeight(int geoX, int geoY, int z, int refIndex)
	{
		...
		switch(type)
		{
			...
			default:
				log.info("GeoEngine: Unknown blockType");
				return z;
		}
	}
	...
	public static byte NgetNSWE(int geoX, int geoY, int z, int refIndex)
	{
		...
		switch(type)
		{
			...
			case BLOCKTYPE_MULTILEVEL:
				...
				while(layers > 0)
				{
					height = (short) ((short) (makeShort(block[index + 1], block[index]) & 0x0fff0) >> 1); // height
					// /
					// 2
					if(height < z_nearest_lower_limit)
					{
	...
	public static void NgetHeightAndNSWE(int geoX, int geoY, short z, short[] result, int refIndex)
	{
		...
		switch(type)
		{
			...
			case BLOCKTYPE_COMPLEX:
				...
				index += (cellX << 3) + cellY << 1;
				height = makeShort(block[index + 1], block[index]);
				result[0] = (short) ((short) (height & 0x0fff0) >> 1); // height
				// / 2
	...
	public static boolean LoadGeodataFile(byte rx, byte ry)
	{
		...
		try
		{
			...
			{
				byte[][][] blocks = new byte[BlocksInMap][1][]; // 256 * 256
				// блоков в
				// регионе
				// геодаты
				...
				for(block = 0; block < blocks.length; block++)
				{
					...
					switch(type)
					{
						...
						case BLOCKTYPE_MULTILEVEL:
							...
							break;
						default:
							log.info("GeoEngine: invalid block type: " + type);
					}
				}
				synchronized(geodata)
				{
	...
	public static void applyControl(GeoControl control)
	{
		...
		@SuppressWarnings("unused") boolean reflectionInit = around == null && refIndex > 0;
		// TODO сделать этот самый Init :)
		// т.е. скопировать кусок геодаты из реального мира в инстанс, еще до
		// закрытия двери.
		if(around == null)
		{
			around = new HashMap<Long, Byte>();
			GArray<Long> around_blocks = new GArray<Long>();
			int minX = pos.getXmin() - World.MAP_MIN_X >> 4;
			int maxX = pos.getXmax() - World.MAP_MIN_X >> 4;
			int minY = pos.getYmin() - World.MAP_MIN_Y >> 4;
			int maxY = pos.getYmax() - World.MAP_MIN_Y >> 4;
		...
		synchronized(around)
		{
			...
			for(long geoXY : around_keys)
			{
				...
				int blockY = getBlock(geoY);
				int blockIndex = getBlockIndex(blockX, blockY);
				// Попытка скопировать блок геодаты, если уже существует, то не
				// скопируется
				if(first_time)
				{
	...
	public static void compact(boolean andClean)
	{
		...
		// TODO
		/*
		 * if(andClean) { Util.gc(2, 100); freeMemBefore = MemoryWatchDog.getMemFree(); }
		 */
		for(int mapX = 0; mapX < World.WORLD_SIZE_X; mapX++)
		{
			for(int mapY = 0; mapY < World.WORLD_SIZE_Y; mapY++)
		...
		// TODO:
		/*
		 * if(andClean) { Util.gc(2, 100); logStr = String.format("%s sOptimized ~%d Mb of memory", logStr, (MemoryWatchDog.getMemFree() -
		 * freeMemBefore) / 0x100000); }
		 */
		log.info(logStr);
	}

...
public class CursedWeaponsManager
{
	...
	private static final int CURSEDWEAPONS_MAINTENANCE_INTERVAL = 5 * 60 * 1000; // 5
	// min
	// in
	// millisec
	...
	private void checkConditions()
	{
		...
		try
		{
			...
			for(CursedWeapon cw : _cursedWeapons)
			{
				...
				// Do an item check to be sure that the cursed weapon and/or
				// skill isn't hold by someone
				int skillId = cw.getSkillId();
				boolean foundedInItems = false;

				// Delete all cursed weapons skills (we don`t care about same
				// skill on multiply weapons, when player back, skill will
				// appears again)
	...
	public void endOfLife(CursedWeapon cw)
	{
		...
		else // either this cursed weapon is in the inventory of someone who has
	...
	public void saveData(CursedWeapon cw)
	{
		Connection con = null;
		PreparedStatement statement = null;
		synchronized(cw)// FIXME [G1ta0] зачем синхронизация если она только на
		// сохранении
		{
			try
			{

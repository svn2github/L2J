...
import l2next.gameserver.model.entity.BlockCheckerEngine;
import l2next.gameserver.model.entity.events.EventType;
import l2next.gameserver.model.entity.events.impl.KrateisCubeRunnerEvent;
...
import l2next.gameserver.network.serverpackets.ExCubeGameChangeTeam;
...
public final class HandysBlockCheckerManager
{
	...
	public boolean addPlayerToArena(Player player, int arenaId)
	{
		...
		synchronized(holder)
		{
			...
			/*
			 * if(UnderGroundColiseum.getInstance().isRegisteredPlayer(player)) { UngerGroundColiseum.getInstance().removeParticipant(player);

package l2next.gameserver.instancemanager.games;
...
/**
 * User: Keiichi Date: 24.11.2008 Time: 23:32:22 Порт с SF.
 */
public class LotteryManager
{
	...
	private void scheduleEndOfLottery()
	{
		...
		/**
		 * Calendar finishtime = Calendar.getInstance(); finishtime.setTimeInMillis(_enddate); finishtime.set(Calendar.MINUTE, 0);
		 * finishtime.set(Calendar.SECOND, 0); finishtime.add(Calendar.DAY_OF_MONTH, 7); finishtime.set(Calendar.DAY_OF_WEEK, 6);
		 * finishtime.set(Calendar.HOUR_OF_DAY, 7); _enddate = finishtime.getTimeInMillis();
		 *
	...
	private class finishLottery extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			// TODO: Уточнить что происходит с джекпотом на оффе. Если с
			// проигрышем всех участников джекпот уменьшается то до каких
			// приделов.

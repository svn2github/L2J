...
import l2next.gameserver.data.xml.holder.CubicHolder;
...
import l2next.gameserver.templates.CubicTemplate;
 ...
 * @date 13:17/22.12.2010 Каждую секунду задача кубика вызывает выбор скила из списка доступных. Если умение на откате (delay), выбирает следующее и
 *       отправляет в исполнение. В обработке умения считается шанс запуска умения (info.getChance()), если скилл не удался - отмена обработки. При
 *       успешной активации проходит обработка умения и оно запускается на откат (delay).
 */
public class EffectCubic extends Effect
{
	...
	public void doAction(Player player)
	{
		...
		{
			if(Rnd.chance(entry.getKey())) // TODO: Должен выбирать один из
			// списка, а не перебирать список
			// шансов
			{
	...
	private static void doHeal(final Player player, CubicTemplate.SkillInfo info, final int delay)
	{
		...
		ThreadPoolManager.getInstance().schedule(new RunnableImpl()
		{
			...
			public void runImpl() throws Exception
			{
				final GArray<Creature> targets = new GArray<Creature>(1);

...
public class Call extends Skill
{
	...
	public void useSkill(Creature activeChar, GArray<Creature> targets)
	{
		...
		for(Creature target : targets)
		{
			if(target != null)
			{
				...
				((Player) target).summonCharacterRequest(activeChar, activeChar.getLoc(), getId() == 1403 || getId() == 1404 ? 1 : 0);
	...
	public static SystemMessage canBeSummoned(Creature target)
	{
		...
		// Нельзя призывать персонажей, которые находятся в режиме PvP или
		// Combat Mode

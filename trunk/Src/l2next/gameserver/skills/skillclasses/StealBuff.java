...
public class StealBuff extends Skill
{
		...
		for(Creature target : targets)
		{
			if(target != null)
			{
				if(calcStealChance(target, activeChar))
				{
					...
					Collections.sort(effectsList, EffectsComparator.getInstance()); // ToFix:
					// Comparator
					// to
					// HF
	...
	private boolean calcStealChance(Creature effected, Creature effector)
	{
		...
		int dml = effector.getLevel() - effected.getLevel(); // to check:
		// magicLevel or
		// player level?
		// Since it's magic
		// skill setting
		// player level as
		// default
		double prelimChance = (dml + 50) * (1 - cancel_res_multiplier * .01); // 50
		// is
		// random
		// reasonable
		// constant

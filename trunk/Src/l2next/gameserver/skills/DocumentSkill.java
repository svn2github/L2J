...
public final class DocumentSkill extends DocumentBase
{
	...
	public class Skill
	{
		...
		public List<l2next.gameserver.model.Skill> skills = new ArrayList<>();
		public List<l2next.gameserver.model.Skill> currentSkills = new ArrayList<>();
	}

	private Skill currentSkill;
	private Set<String> usedTables = new HashSet<>();
	private List<l2next.gameserver.model.Skill> skillsInFile = new LinkedList<>();
	...
	protected List<l2next.gameserver.model.Skill> getSkills()
	{
		return skillsInFile;
	}
	...
	protected void parseSkill(Node n)
	{
		...
		try
		{
			...
			for(int k = 0; k < d.getChildNodes().getLength(); k++)
			{
				...
				String el = enchant.getNodeName().substring("enchant".length());
				int enchantLevel = Integer.parseInt(el);

				String ename = enchant.getAttributes().getNamedItem("name").getNodeValue();
				for(int r = 1; r <= eLevels; r++)
				{
					if(count != enchantLevel - 1)
					{
						count = enchantLevel - 1;
					}
			...
			// РѕР±СЂР°Р±Р°С‚С‹РІР°РµРј С‚Р°Р±Р»РёС†С‹ СЃР»РёРІР°СЏ РёС… СЃ
			// СЌРЅС‡Р°РЅС‚Р°РјРё
			...
			for(int i = 0; i < lastLvl; i++)
			{
				...
				l2next.gameserver.model.Skill current = currentSkill.currentSkills.get(i);

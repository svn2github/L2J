...
import l2next.gameserver.utils.AdminFunctions;
		...
		if(banChatCountPerDay > 0)
		{
			...
			if(LastBanChatDayTime != 0)
			{
				...
				else
				{
					...
					bonus_mod = 1; // Убрать, если потребуется сделать
					// зависимость бонуса от количества банов
					if(activeChar.getPlayerAccess().BanChatBonusId > 0 && activeChar.getPlayerAccess().BanChatBonusCount > 0)
					{
						...
						if(penaltyCount > 0) // У модератора был штраф за
						// нарушения
						{
							activeChar.sendMessage("Штраф за нарушения: " + penaltyCount + " " + item.getName());
							activeChar.setVar("penaltyChatCount", "" + Math.max(0, penaltyCount - add_count), -1); // Уменьшаем
							// штраф
							add_count -= penaltyCount; // Вычитаем штраф из
							// бонуса
						}

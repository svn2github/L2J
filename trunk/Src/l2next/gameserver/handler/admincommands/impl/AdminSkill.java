...
public class AdminSkill implements IAdminCommandHandler
{
	...
	private void adminResetSkills(Player activeChar)
	{
		...
		// TODO WtF?
		/*
		 * for(L2Skill element : skills) if(!element.isCommon() && !SkillTreeTable.getInstance().isSkillPossible(player, element.getId(),
		 * element.getLevel())) { player.removeSkill(element, true); counter++; }
		 */
		player.checkSkills();
		player.sendSkillList();

...
public class AdminGmChat implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			{
				/**
				 * L2Object target = activeChar.getTarget(); if(target == null) { activeChar.sendMessage("You must select a target."); return false; }
				 * if(!target.isPlayer) { activeChar.sendMessage("Target must be a player."); return false; } L2Player player = (L2Player) target;
				 * player.addSnooper(activeChar); activeChar.addSnooped(player);
				 */
			}
		}
		return true;

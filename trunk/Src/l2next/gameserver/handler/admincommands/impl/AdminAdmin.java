...
import l2next.gameserver.network.serverpackets.ExChangeClientEffectInfo;
		...
		if(activeChar.getPlayerAccess().Menu)
		{
			switch(command)
			{
				...
				case admin_play_sound:
					...
					break;
				case admin_silence:
					if(activeChar.getMessageRefusal()) // already in message
					// refusal
					// mode
					{
				...
				case admin_locdump:
					...
					try
					{
						...
						try
						{
							...
							writer.write("(" + activeChar.getLoc().x + ", " + activeChar.getLoc().y + ", " + activeChar.getLoc().z + "," + activeChar.getLoc().h + ")" + "\n");
							writer.close();
						}
						catch(Exception e)
						{
				...
				case admin_zonedump:
					...
					try
					{
						...
						{
							new File("dumps").mkdir();
							File f = new File("dumps/locdumpZone.txt");
							if(!f.exists())
							{
								f.createNewFile();
							}

...
public class AdminClanHall implements IAdminCommandHandler
{
	...
	public void showClanHallSelectPage(Player activeChar)
	{
		...
		/*
		 * for(ClanHall clanhall : ClanHallManager.getInstance().getClanHalls().values()) if(clanhall != null) { replyMSG.append("<tr><td>");
		 ...
		 * 
		 * L2Clan owner = clanhall.getOwnerId() == 0 ? null : ClanTable.getInstance().getClan(clanhall.getOwnerId()); if(owner == null)
		 * replyMSG.append("none"); else replyMSG.append(owner.getName());
		 * 

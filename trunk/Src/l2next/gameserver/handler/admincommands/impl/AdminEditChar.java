...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if(fullString.startsWith("admin_setsubclass"))
		{
			...
			if(st.countTokens() > 1)
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", activeChar));
					return false;
				}
				player.sendPacket(Msg.CONGRATULATIONS_YOU_HAVE_TRANSFERRED_TO_A_NEW_CLASS); // Transfer
				// to
				// new
				// class.
			}
	...
	{
		GArray<Player> players = GameObjectsStorage.getAllPlayers();
	...
	private void setSubclass(final Player activeChar, final Player player)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2next.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", activeChar));
			return;
		}

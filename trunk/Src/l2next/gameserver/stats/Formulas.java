package l2next.gameserver.stats;
...
public class Formulas
{
	...
	/**
	 * Для простых ударов patk = patk При крите простым ударом: patk = patk * (1 + crit_damage_rcpt) * crit_damage_mod + crit_damage_static Для blow
	 * скиллов TODO Для скилловых критов, повреждения просто удваиваются, бафы не влияют (кроме blow, для них выше) patk = (1 + crit_damage_rcpt) *
	 * (patk + skill_power) Для обычных атак damage = patk * ss_bonus * 70 / pdef
	 */
	public static AttackInfo calcPhysDam(Creature attacker, Creature target, Skill skill, boolean dual, boolean blow, boolean ss, boolean onCrit)
	{
		...
		if(skill != null)
		{
			if(!blow && !target.isLethalImmune()) // считаем леталы для не blow
			// cкиллов
			{
			...
			// если скилл не имеет своей силы дальше идти бесполезно, можно
			// сразу вернуть дамаг от летала
			...
			if(blow && !skill.isBehind() && ss) // Для обычных blow не влияет
			// на power
			{
				info.damage *= 2.04;
			}
			...
			if(blow && skill.isBehind() && ss) // Для backstab влияет на power,
			// но меньше множитель
			{
				info.damage *= 1.5;
			}
		...
		if(info.crit)
		{
			// шанс абсорбации души (без анимации) при крите, если Soul Mastery
			// 4го уровня или более
		...
		info.damage += attacker.calcStat(Stats.PHYS_SKILL_DMG_STATIC, /*
																	 * info.damage
																	 */1, target, skill);
		...
		// Получение только определенного кол-ва урона, используется для скилов
	...
	public static AttackInfo calcMagicDam(Creature attacker, Creature target, Skill skill, int sps)
	{
		...
		if(info.crit)
		{
			info.damage *= attacker.calcStat(Stats.MCRITICAL_DAMAGE, attacker.isPlayable() && target.isPlayable() ? 2.5 : 3., target, skill);
			info.damage += attacker.calcStat(Stats.CRITICAL_DAMAGE_STATIC, target, skill);
		}
		...
		int levelDiff = target.getLevel() - attacker.getLevel(); // C Gracia
		// Epilogue
		// уровень
		// маг. атак
		// считается
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		if(lvlDependMod != 0)
		{
			...
			/*
			 * final int delta = attackLevel - target.getLevel(); lvlDependMod = delta / 5; lvlDependMod = lvlDependMod * 5; if (lvlDependMod !=
			 * delta) lvlDependMod = delta < 0 ? lvlDependMod - 5 : lvlDependMod + 5;
			 * 
		...
		env.value = Math.max(env.value, Math.min(base, Config.SKILLS_CHANCE_MIN)); // Если
		// базовый
		// шанс
		// более
	...
	public static void calcSkillMastery(Skill skill, Creature activeChar)
	{
		...
		if(activeChar.getSkillLevel(331) > 0 && activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getINT(), null, skill) >= Rnd.get(1000) || (activeChar.getSkillLevel(330) > 0 || activeChar.getSkillLevel(10501) > 0) && activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getSTR(), null, skill) >= Rnd.get(1000))
		{
			...
			if(skill.isMusic() || type == Skill.SkillType.BUFF || type == Skill.SkillType.HOT || type == Skill.SkillType.HEAL_PERCENT) // Hope
			// i
			// didn't
			// forget
			// skills
	...
	{
		if(attacker == defender) // это дамаг от местности вроде ожога в лаве,
		// наносится от своего имени
		{
			return value; // TODO: по хорошему надо учитывать защиту, но
		}
		// поскольку эти скиллы немагические то надо делать
		// отдельный механизм
		...
		// если уровень игрока ниже чем на 2 и более уровней моба 78+, то его
		// урон по мобу снижается
	...
	/**
	 * Возвращает множитель для атаки из значений атакующего и защитного элемента. <br />
	 * <br />

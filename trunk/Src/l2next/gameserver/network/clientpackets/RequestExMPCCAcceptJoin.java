	...
	protected void runImpl()
	{
		...
		try
		{
			...
			else if(CommandChannel.checkAuthority(requestor))
			{
				...
				// Скила нету, придется расходовать предмет, ищем Strategy Guide
				// в инвентаре
				if(!haveSkill)
				{
				...
				CommandChannel channel = new CommandChannel(requestor); // Создаём
				// Command
				// Channel
				requestor.sendPacket(Msg.THE_COMMAND_CHANNEL_HAS_BEEN_FORMED);
				channel.addParty(activeChar.getParty()); // Добавляем
				// приглашенную партию
			}
		}
		finally

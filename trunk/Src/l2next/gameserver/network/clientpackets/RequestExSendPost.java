...
public class RequestExSendPost extends L2GameClientPacket
{
	...
	protected void readImpl()
	{
		...
		if(_count * 12 + 4 > _buf.remaining() || _count > Short.MAX_VALUE || _count < 1) // TODO
		// [G1ta0]
		// audit
		{
			_count = 0;
	...
	protected void runImpl()
	{
		...
		if(activeChar.isInBlockList(_recieverName)) // тем кто в блоклисте не
		// шлем
		{
		...
		if(target != null)
		{
			...
			if(target.isInBlockList(activeChar)) // цель заблокировала
			// отправителя
			{
		...
		else
		{
			...
			{
				if(mysql.simple_get_int("target_Id", "character_blocklist", "obj_Id=" + recieverId + " AND target_Id=" + activeChar.getObjectId()) > 0) // цель
				// заблокировала
				// отправителя
				{
					activeChar.sendPacket(new SystemMessage(SystemMessage.S1_HAS_BLOCKED_YOU_YOU_CANNOT_SEND_MAIL_TO_S1_).addString(_recieverName));
		...
		int expireTime = (_messageType == 1 ? 12 : 360) * 3600 + (int) (System.currentTimeMillis() / 1000L); // TODO
		// [G1ta0]
		// хардкод
		// времени
		// актуальности
		...
		long serviceCost = 100 + _count * 1000; // TODO [G1ta0] хардкод цена за
		// почту

...
{
	private static final long _FREIGHT_FEE = 1000; // TODO [VISTALL] hardcode
	// price
	...
	protected void runImpl() throws Exception
	{
		...
		try
		{
			...
			// Создаем новый список передаваемых предметов, на основе полученных
			// данных
			for(int i = 0; i < _count; i++)
			{
				...
				if(!item.isStackable() || freight.getItemByItemId(item.getItemId()) == null) // вещь
				// требует
				// слота
				{
					if(slotsleft <= 0) // если слоты кончились нестекуемые вещи
					// и отсутствующие стекуемые пропускаем
					{
						_items[i] = 0; // Обнуляем, вещь не будет передана
						_itemQ[i] = 0L;
						continue;

...
public class RequestPledgeSetMemberPowerGrade extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if((activeChar.getClanPrivileges() & Clan.CP_CL_MANAGE_RANKS) == Clan.CP_CL_MANAGE_RANKS)
		{
			...
			{
				activeChar.sendMessage(new CustomMessage("l2next.gameserver.clientpackets.RequestPledgeSetMemberPowerGrade.NotBelongClan", activeChar));
			}
		}
		else
		{
			activeChar.sendMessage(new CustomMessage("l2next.gameserver.clientpackets.RequestPledgeSetMemberPowerGrade.HaveNotAuthority", activeChar));
		}
	}
}
...
import l2next.gameserver.network.serverpackets.ExAttributeEnchantResult;
...
public class RequestEnchantItemAttribute extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		AttributeStone asi = EnchantItemHolder.getInstance().getAttributeStone(stone.getItemId());
		...
		if(Rnd.chance(asi.getChance()))
		{
			...
			// Для оружия 1й камень дает +20 атрибута, исключаем новые атрибуты "Super".
			if(currentValue == 0 && itemToEnchant.isWeapon() && !asi.isSuper())
			{
				value = 20;
			}
			...
			itemToEnchant.setAttributeElement(element, Math.min(currentValue + value, maxValue));

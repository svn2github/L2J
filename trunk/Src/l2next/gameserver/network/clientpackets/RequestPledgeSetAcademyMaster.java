...
public class RequestPledgeSetAcademyMaster extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if((activeChar.getClanPrivileges() & Clan.CP_CL_APPRENTICE) == Clan.CP_CL_APPRENTICE)
		{
			...
			if(sponsor != null && apprentice != null)
			{
				...
				if(_mode == 1)
				{
					...
					{
						activeChar.sendMessage(new CustomMessage("l2next.gameserver.clientpackets.RequestOustAlly.MemberAlreadyHasApprentice", activeChar));
						return;
					}
					if(apprentice.hasSponsor())
					{
						activeChar.sendMessage(new CustomMessage("l2next.gameserver.clientpackets.RequestOustAlly.ApprenticeAlreadyHasSponsor", activeChar));
						return;
					}
				...
				else
				{
					if(!sponsor.hasApprentice())
					{
						activeChar.sendMessage(new CustomMessage("l2next.gameserver.clientpackets.RequestOustAlly.MemberHasNoApprentice", activeChar));
						return;
					}
		...
		{
			activeChar.sendMessage(new CustomMessage("l2next.gameserver.clientpackets.RequestOustAlly.NoMasterRights", activeChar));
		}
	}
}
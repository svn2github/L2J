...
/**
 * Запрос на удаление отправленных сообщений. Удалить можно только письмо без вложения. Отсылается при нажатии на "delete" в списке отправленных
 * писем.
 *
			...
			for(Mail mail : mails)
			{
				if(ArrayUtils.contains(_list, mail.getMessageId()))
				{
					if(mail.getAttachments().isEmpty())
						// FIXME [G1ta0] если почта не прочитана получателем,
						// возможно имеет смысл удалять ее совсем, на офф.
						// сервере не удаляется.
						/*

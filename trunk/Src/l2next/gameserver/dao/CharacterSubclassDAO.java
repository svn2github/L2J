...
import l2next.gameserver.model.DeathPenalty;
...
/**
 * Created with IntelliJ IDEA. User: Darvin Date: 24.04.12 Time: 18:56
 */
public class CharacterSubclassDAO
{
	...
	public static final String SELECT_SQL_QUERY = "SELECT class_id, exp, sp, curHp, curCp, curMp, active, type, death_penalty, certification FROM character_subclasses WHERE char_obj_id=?";
	public static final String INSERT_SQL_QUERY = "INSERT INTO character_subclasses (char_obj_id, class_id, exp, sp, curHp, curMp, curCp, maxHp, maxMp, maxCp, level, active, type, death_penalty, certification) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean insert(int objId, int classId, long exp, int sp, double curHp, double curMp, double curCp, double maxHp, double maxMp, double maxCp, int level, boolean active, SubClassType type, DeathPenalty deathPenalty, int certification)
	{
		Connection con = null;
		...
		try
		{
			...
			statement.setInt(14, deathPenalty == null ? 0 : deathPenalty.getLevelOnSaveDB());

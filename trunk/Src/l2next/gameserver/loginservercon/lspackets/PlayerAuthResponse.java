...
import l2next.gameserver.loginservercon.gspackets.PlayerInGame;
...
import l2next.gameserver.network.serverpackets.LoginFail;
	...
	protected void runImpl()
	{
		...
		if(authed && client.getSessionKey().equals(skey))
		{
			...
			if(oldClient != null)
			{
				...
				if(activeChar != null)
				{
					// FIXME [G1ta0] сообщение чаще всего не показывается, т.к.
					// при закрытии соединения очередь на отправку очищается

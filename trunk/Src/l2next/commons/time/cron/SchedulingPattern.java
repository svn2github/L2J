 ...
 */
package l2next.commons.time.cron;
...
public class SchedulingPattern
{
	...
	/**
	 * This methods returns next matching timestamp (expressed as a UNIX-era millis value) according the pattern and given timestamp.
	 *
	 * @param timezone
	 *            A time zone.
	 * @param millis
	...
	/**
	 * <p>
	 * A ValueMatcher whose rules are in a plain array of integer values. When asked to validate a value, this ValueMatcher checks if it is in the
	 * array.
	 * </p>
	 */
	private static class IntArrayValueMatcher implements ValueMatcher
	{
		 ...
		 *
		 * @param integers
		 *            An ArrayList<Integer> of Integer elements, one for every value accepted by the matcher. The match() method will return true only
		 *            if its parameter will be one of this list.
		 */
	 ...
	 * <p>
	 * A ValueMatcher whose rules are in a plain array of integer values. When asked to validate a value, this ValueMatcher checks if it is in the
	 * array and, if not, checks whether the last-day-of-month setting applies.
	 * </p>
	 */
	private static class DayOfMonthValueMatcher extends IntArrayValueMatcher
	{
		 ...
		 * @param integers
		 *            An ArrayList<Integer> of Integer elements, one for every value accepted by the matcher. The match() method will return true only
		 *            if its parameter will be one of this list or the last-day-of-month setting applies.
		 */

...
public class SelectorThread<T extends MMOClient> extends Thread
{
	...
	protected void readPacket(SelectionKey key)
	{
		...
		// if we try to to do a read with no space in the buffer it will read 0
		// bytes
	...
	protected boolean tryReadPacket2(SelectionKey key, MMOConnection<T> con, ByteBuffer buf)
	{
		...
		// проверяем, хватает ли нам байт для чтения заголовка и не пустого тела
		// пакета
		...
		// не хватает данных, переносим содержимое первичного буфера во
		// вторичный
		if(buf == READ_BUFFER)
		{
		...
		if(DIRECT_WRITE_BUFFER.hasRemaining() && !con.hasPendingWriteBuffer())
		{
			...
			for(i = 0; i < _sc.MAX_SEND_PER_PASS; i++)
			{
				...
				try
				{
					...
					putPacketIntoWriteBuffer(sp, true); // записываем пакет в
					// WRITE_BUFFER
					WRITE_BUFFER.flip();
					...
					// если не осталось места в DIRECT_WRITE_BUFFER для
					// WRITE_BUFFER то мы его запишев в следующий раз
					{

...
import l2next.gameserver.network.serverpackets.ExShowTrace;
...
public class ItemBroker extends Functions
{
	...
	public String getHtmlAppends(Integer val)
	{
		...
		if(type > 0)
		{
			if(getSelf().isLangRus())
			{
				...
				if(type != Player.STORE_PRIVATE_MANUFACTURE)
				{
					append.append("[scripts_services.ItemBroker:list ").append(type).append(" 90 1 0 0|<font color=\"FF9900\">Стандартные предметы</font>]<br1>"); // отсутствующий
				}
				// элемент

				append.append("<edit var=\"tofind\" width=100><br1>");
				append.append("[scripts_services.ItemBroker:find ").append(type).append(" 1 $tofind|<font color=\"FF9900\">Найти</font>]<br1>"); // DS:
				// разобраться
				// как
				// и
				// почему
			...
			else
			{
				...
				append.append("<edit var=\"tofind\" width=100><br1>");
				append.append("[scripts_services.ItemBroker:find ").append(type).append(" 1 $tofind|<font color=\"FF9900\">Find</font>]<br1>"); // DS:
				// разобраться
				// как
				// и
				// почему
	...
	public void updateInfo(Player player, NpcInstance npc)
	{
		...
		if(info == null || info.lastUpdate < System.currentTimeMillis() - 300000)
		{
			...
			for(Player pl : World.getAroundPlayers(npc, 4000, 400))
			{
				...
				{
					// if(GeoEngine.canMoveToCoord(npc.getX(), npc.getY(), npc.getZ(), pl.getX(), pl.getY(), pl.getZ(), npc.getGeoIndex()))
					...
					switch(type)
					{
						...
						case Player.STORE_PRIVATE_MANUFACTURE:
							...
							for(ManufactureItem mitem : createList)
							{
								...
								if(recipe == null || recipe.getProductions().isEmpty())
								{
									continue;
								}

								for(RecipeProduction product : recipe.getProductions())
								{
	...
	public void find(String[] var)
	{
		...
		for(Entry<String, TreeMap<Long, Item>> entry : allItems.entrySet())
		{
			...
			// Ищем первый подходящий предмет
			{

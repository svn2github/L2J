...
public class Teredor extends Fighter
{
	...
	{
		/* FIXME: раскоментировать, когда будет реализован скилл Jump Attack.
		Червяк сразу после агра бьет прыжком 1 раз.
		if (!_jumpAttacked){
			if (!actor.isCastingNow())
				actor.doCast(SkillHolder.getInstance().getInfo(jumpSkillId, 1), actor, true);
		}*/

		/* FIXME: раскоментировать, когда будет реализован скилл Poison.
		if (!_canUsePoison){
			if (!actor.isCastingNow() && !_poisonCasted)
			{
				actor.doCast(SkillHolder.getInstance().getInfo(poisonSkillId, 1), actor, true);
				_poisonCasted = true;
			}
		}*/
		...
		if(!_battleActive)
		{
			...
			ThreadPoolManager.getInstance().scheduleAtFixedRate(new EggSpawnTask(r), 1000, delayEggTask * 1000);
		}

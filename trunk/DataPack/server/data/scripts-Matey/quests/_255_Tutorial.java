...
import l2next.gameserver.network.serverpackets.TutorialCloseHtml;
...
public class _255_Tutorial extends Quest implements ScriptFile, OnPlayerEnterListener
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.startsWith("UC"))
		{
			...
			else if(level >= 76 && player.getQuestState("_10341_DayOfDestinyHumansFate") == null && player.getClassLevel() == 3 && player.getRace() == Race.human)
			{
				st.showQuestionMark(101);
				st.playSound(SOUND_TUTORIAL);
			}
			else if(level >= 76 && player.getQuestState("_10342_DayOfDestinyElvenFate") == null && player.getClassLevel() == 3 && player.getRace() == Race.elf)
			{
				st.showQuestionMark(101);
				st.playSound(SOUND_TUTORIAL);
			}
			else if(level >= 76 && player.getQuestState("_10343_DayOfDestinyDarkElfsFate") == null && player.getClassLevel() == 3 && player.getRace() == Race.darkelf)
			{
				st.showQuestionMark(101);
				st.playSound(SOUND_TUTORIAL);
			}
			else if(level >= 76 && player.getQuestState("_10344_DayOfDestinyOrcsFate") == null && player.getClassLevel() == 3 && player.getRace() == Race.orc)
			{
				st.showQuestionMark(101);
				st.playSound(SOUND_TUTORIAL);
			}
			else if(level >= 76 && player.getQuestState("_10345_DayOfDestinyDwarfsFate") == null && player.getClassLevel() == 3 && player.getRace() == Race.dwarf)
			{
				st.showQuestionMark(101);
				st.playSound(SOUND_TUTORIAL);
			}
			else if(level >= 76 && player.getQuestState("_10346_DayOfDestinyKamaelFate") == null && player.getClassLevel() == 3 && player.getRace() == Race.kamael)
			{
				st.showQuestionMark(101);
				st.playSound(SOUND_TUTORIAL);
			}
		...
		else if(event.startsWith("CE"))
		{
			...
			else if(event_id == 40)
			{
				...
				{
					if(st.getInt("lvl") < 76 && player.getClassLevel() == 3)
					{
						st.playSound(SOUND_TUTORIAL);
						st.set("lvl", "76");
						st.showQuestionMark(101);
				...
				{
					if(st.getInt("lvl") < 85 && player.getClassLevel() == 4)
					{
						st.playSound(SOUND_TUTORIAL);
						st.set("lvl", "85");
						st.getPlayer().sendPacket(new ExShowScreenMessage(NpcString.FREE_THE_GIANT_FROM_HIS_IMPRISONMENT_AND_AWAKEN_YOUR_TRUE_POWER, 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, ExShowScreenMessage.STRING_TYPE, 0, false, 0));
						st.getPlayer().sendPacket(new TutorialShowHtml(TutorialShowHtml.AWAKING85_90, TutorialShowHtml.TYPE_WINDOW));
						AwakingManager.getInstance().SendReqToStartQuest(st.getPlayer());
					}
				}
				else if(player.getLevel() == 90)
				{
					if(st.getInt("lvl") < 90 && player.getClassLevel() == 5)
					{
						st.playSound(SOUND_TUTORIAL);
						st.set("lvl", "90");
				...
				{
					if(st.getInt("lvl") < 95 && player.getClassLevel() == 5)
					{
						st.playSound(SOUND_TUTORIAL);
						st.set("lvl", "95");

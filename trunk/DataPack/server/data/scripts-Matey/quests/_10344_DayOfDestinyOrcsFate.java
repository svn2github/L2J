 ...
 * @name 10344 - Day of Destiny Orcs Fate
 * @category Class quest. One
 * @see http://l2wiki.com/Day_of_Destiny:_Orc%27s_Fate
 */
public class _10344_DayOfDestinyOrcsFate extends Quest implements ScriptFile
{
	...
	private static final int[] ADEN_VANGUARD_CORPSE = {
		33166,
		33167,
		33168,
		33169
	...
	public _10344_DayOfDestinyOrcsFate()
	{
		super(false);
		addStartNpc(LADANZA);
		addTalkId(LADANZA, QUARTERMASTER, ADEN_VANGUARD_MEMBER, ADOLPH);
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("corpse_search"))
		{
		...
		else if(event.equalsIgnoreCase("quest_select_helper"))
		{
			st.setCond(6);
			st.playSound(SOUND_MIDDLE);
		...
		else if(event.equalsIgnoreCase("select_helper"))
		{
		...
		else if(event.equalsIgnoreCase("start_instance"))
		{
			st.setCond(8);
			st.playSound(SOUND_MIDDLE);
		...
		else if(event.equalsIgnoreCase("take_item"))
		{
		...
		else if(event.equalsIgnoreCase("start_second_part"))
		{
			st.setCond(11);
			st.playSound(SOUND_MIDDLE);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == LADANZA)
		{
			...
			else if(cond == 0 && race != Race.orc)
			{
				htmltext = "30865-02.htm";
			}
			else if(!isAvailableFor(player))
			{
				htmltext = "30865-10.htm";
			}
			else if(cond == 0)
			{
			...
			{
				htmltext = "30865-08.htm";
			}
			else if(cond == 13)
			{
				htmltext = "30865-09.htm";
			}
			else
			{
				htmltext = "30865-08.htm";

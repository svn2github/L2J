...
public class FortunaInstance extends Reflection
{
	...
	private class DeathListener implements OnDeathListener
	{
		...
		public void onDeath(Creature self, Creature killer)
		{
			for(Player p : getPlayers())
			{
				...
				if((self.isNpc()) && (self.getNpcId() == 23077) && p.getVar("Fortuna1") != null)
				{
					ThreadPoolManager.getInstance().schedule(new FortunaInstance.SelphinaSpawn(), 10000L);
					self.deleteMe();
					p.unsetVar("Fortuna1");
				}
				else if((self.isNpc()) && (self.getNpcId() == 23076) && p.getVar("Fortuna2") != null)
				{
				...
				else if((self.isNpc()) && (self.getNpcId() == 23081) && p.getVar("Fortuna3") != null)
				{
	...
	private class BonusStageMain extends RunnableImpl
	{
		...
		{
			/* 84 */
			ThreadPoolManager.getInstance().schedule(new FortunaInstance.BonusStageMainMob1SubStage(), 10000L);
		}
	}
	...
	private class FreeStageMainMob1SubStage extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			{
				/* 45 */
				player.sendPacket(new ExShowScreenMessage(NpcString.STAGE_3, 6000, ExShowScreenMessage.ScreenMessageAlign.BOTTOM_RIGHT, true, 1, -1, true, new String[0]));
			}
			/* 47 */
			addSpawnWithoutRespawn(23078, new Location(41448, -175608, -7974), 0);
			/* 48 */
	...
	private class FreeStageMain extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			{
				/* 33 */
				player.sendPacket(new ExShowScreenMessage(NpcString.POSMOTRIM, 6000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true, new String[0]));
			}
			/* 35 */
			ThreadPoolManager.getInstance().schedule(new FortunaInstance.FreeStageMainMob1SubStage(), 10000L);
		}
	}
	...
	private class EndTwoStageMob extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			NpcInstance voitelSpecial = addSpawnWithoutRespawn(23076, new Location(42104, -176344, -7974, 15956), 0);
			/* 22 */
	...
	private class TwoStageMainMob1SubStage extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			{
				/* 47 */
				player.sendPacket(new ExShowScreenMessage(NpcString.STAGE_2, 6000, ExShowScreenMessage.ScreenMessageAlign.BOTTOM_RIGHT, true, 1, -1, true, new String[0]));
			}
			/* 49 */
			addSpawnWithoutRespawn(23077, new Location(41448, -175608, -7974), 0);
			/* 50 */
	...
	private class TwoStageMain extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			{
				/* 35 */
				player.sendPacket(new ExShowScreenMessage(NpcString.AETTEKTO, 6000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true, new String[0]));
			}
			/* 37 */
			ThreadPoolManager.getInstance().schedule(new FortunaInstance.TwoStageMainMob1SubStage(), 30000L);
		}
	}
	...
	private class EndOneStageMob extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			for(Player p : getPlayers())
			{
				p.setVar("Fortuna1", "@Kill", -1);
			}
			NpcInstance yarostSpecial = addSpawnWithoutRespawn(23077, new Location(42104, -176344, -7974, 15956), 0);
			/* 89 */

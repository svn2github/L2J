...
public class _421_LittleWingAdventures extends Quest implements ScriptFile
{
	...
	{
		Summon _pet = st.getPlayer().getFirstPet();
		if(_pet == null)
		{
			return false;
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Mimyu)
		{
			...
			if(cond == 3)
			{
				...
				if(HatchlingSummoned(st, false) || st.getPlayer().getFirstPet() != null)
				{
	...
	public class GuardiansSpawner extends RunnableImpl
	{
		...
		public GuardiansSpawner(NpcInstance npc, QuestState st, int _count)
		{
			...
			for(int i = 0; i < _count; i++)
			{
				...
				if(st.getPlayer().getFirstPet() != null)
				{
					agressors_pet = st.getPlayer().getFirstPet().getName();
				}
		...
		private void updateAgression()
		{
			...
			if(_player != null)
			{
				if(agressors_pet != null && _player.getFirstPet() != null && _player.getFirstPet().getName().equalsIgnoreCase(agressors_pet))
				{
					AddAgression(_player.getFirstPet(), 10);
				}

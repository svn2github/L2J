...
import l2next.gameserver.model.AggroList.HateComparator;
...
public class OnActionShift extends Functions
{
	public boolean OnActionShift_NpcInstance(Player player, GameObject object)
	{
		...
		if(object.isNpc())
		{
			...
			if(Config.ALT_FULL_NPC_STATS_PAGE)
			{
				...
				dialog = dialog.replaceFirst("%killed%", String.valueOf(0));// TODO
				// [G1ta0]
				// убрать
	...
	public boolean OnActionShift_ItemInstance(Player player, GameObject object)
	{
		...
		if(object.isItem())
		{
			...
			Player owner = GameObjectsStorage.getPlayer(item.getOwnerId()); // FIXME
			// [VISTALL]
			// несовсем
			// верно,
			// может
			...
			// dialog = dialog.replaceFirst("%dropOwner%",
			// String.valueOf(item.getDropOwnerId()));
			// dialog = dialog.replaceFirst("%dropOwnerId%",
			// String.valueOf(item.getDropOwnerId()));

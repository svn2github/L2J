...
public class LabyrinthOfHarnak extends DynamicQuest implements ScriptFile
{
	...
	private static final int CLIENT_PARAM = 801;
	...
	public LabyrinthOfHarnak()
	{
		...
		addTask(CLIENT_PARAM, MAX_TASK_POINT, TASK_INCREASE_MODE_NO_LIMIT);
		addReward(REWARD, 1);
	...
	{
		ThreadPoolManager.getInstance().schedule(new enter(), 10 * 1000);
	}
	
	private class enter extends RunnableImpl
	{
		@Override
		public void runImpl() throws Exception
		{
			DynamicQuestController.getInstance().startQuest(QUEST_ID,801);
		}
	}
	...
	private final class KillListenerImpl implements OnKillListener
	{
		...
		public void onKill(Creature actor, Creature victim)
		{
			...
			if(victim.isNpc() && isStarted() && ArrayUtils.contains(LOH_MOBS, victim.getNpcId()))
			{
				...
				{
					case DEMONIC_BATHUS:
						increaseTaskPoint(CLIENT_PARAM, actor.getPlayer(), 1000);
						break;
					...
					case DEMONIC_WEISS_ELE:
						increaseTaskPoint(CLIENT_PARAM, actor.getPlayer(), 1);
						break;
					...
					case DEMONIC_SEKNUS:
						...
						break;
					case DEMONIC_NOKTUM:
						increaseTaskPoint(CLIENT_PARAM, actor.getPlayer(), 2);
						break;
				}
			}
		}

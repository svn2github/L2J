...
import l2next.gameserver.model.entity.events.objects.AuctionSiegeClanObject;
...
public class AuctioneerInstance extends NpcInstance
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -5123101752420675425L;
		...
		else if(actualCommand.equalsIgnoreCase("info"))
		{
			...
			else
			{
				...
				else
				{
					for(ClanHall ch : ResidenceHolder.getInstance().getResidenceList(ClanHall.class))
					{
						if(ch.getSiegeEvent().getClass() == ClanHallAuctionEvent.class && (siegeClan = ch.getSiegeEvent().getSiegeClan(SiegeEvent.ATTACKERS, player.getClan())) != null)
						{
							clanHall = ch;
							break;
						}

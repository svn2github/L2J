...
import l2next.gameserver.listener.actor.player.OnTeleportListener;
...
import l2next.gameserver.network.serverpackets.ExCubeGameAddPlayer;
import l2next.gameserver.network.serverpackets.ExCubeGameChangePoints;
import l2next.gameserver.network.serverpackets.ExCubeGameCloseUI;
import l2next.gameserver.network.serverpackets.ExCubeGameEnd;
import l2next.gameserver.network.serverpackets.ExCubeGameExtendedChangePoints;
import l2next.gameserver.network.serverpackets.ExCubeGameRemovePlayer;
...
/**
 * Р�РЅСЃС‚Р°РЅСЃ РґР»СЏ GvG С‚СѓСЂРЅРёСЂР°
 *
 * @author pchayka
 */
public class GvGInstance extends Reflection
{
	...
	private void end()
	{
		...
		//РЈРґР°Р»РµРЅРёРµ СЃРѕР·РґР°РЅРЅС‹С… Р·РѕРЅ РёР· РјРёСЂР°
	...
	private void reward(Party party)
	{
		...
		{
			member.sendMessage("Р’Р°С€Р° РіСЂСѓРїРїР° РІС‹РёРіСЂР°Р»Р° GvG С‚СѓСЂРЅРёСЂ, Р»РёРґРµСЂ РіСЂСѓРїРїС‹ РґРѕР±Р°РІР»РµРЅ РІ СЂРµР№С‚РёРЅРі РїРѕР±РµРґРёС‚РµР»РµР№.");
	...
	private class DeathListener implements OnDeathListener
	{
		...
		public void onDeath(Creature self, Creature killer)
		{
			...
			//РЈР±РёР№СЃС‚РІРѕ РїСЂРѕРёР·РѕС€Р»Рѕ РІ РёРЅСЃС‚Р°РЅС‚Рµ
			...
			else if(self.isNpc() && killer.isPlayable())
			{
				...
				else if(self.getNpcId() == BOSS_ID)
				{
					...
					broadCastPacketToBothTeams(new ExShowScreenMessage("РћС…СЂР°РЅРЅРёРє РЎРѕРєСЂРѕРІРёС‰ Р“РµСЂР°Р»СЊРґР° РїРѕРіРёР± РѕС‚ СЂСѓРєРё " + killer.getName(), 5000, ScreenMessageAlign.MIDDLE_CENTER, true));
					end();
				}
			}
		}
	...
	private void teamWithdraw(Party party)
	{
		...
		broadCastPacketToBothTeams(new ExShowScreenMessage("РљРѕРјР°РЅРґР° СЃРѕРїРµСЂРЅРёРєР° РїРѕРєРёРЅСѓР»Р° РїРѕР»Рµ Р±РѕСЏ РІ РїРѕР»РЅРѕРј СЃРѕСЃС‚Р°РІРµ. РљРѕРЅРµС† СЃСЂР°Р¶РµРЅРёСЏ.", 4000, ScreenMessageAlign.MIDDLE_CENTER, true));
		end();
	}
	...
	public class BossSpawn extends RunnableImpl
	{
		...
		{
			broadCastPacketToBothTeams(new ExShowScreenMessage("РџРѕСЏРІРёР»СЃСЏ РћС…СЂР°РЅРЅРёРє РЎРѕРєСЂРѕРІРёС‰ Р“РµСЂР°Р»СЊРґР°", 5000, ScreenMessageAlign.MIDDLE_CENTER, true));
	...
	public class CountingDown extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			broadCastPacketToBothTeams(new ExShowScreenMessage("Р”Рѕ РєРѕРЅС†Р° СЃСЂР°Р¶РµРЅРёСЏ РѕСЃС‚Р°Р»Р°СЃСЊ 1 РјРёРЅСѓС‚Р°", 4000, ScreenMessageAlign.MIDDLE_CENTER, true));
		}
	}
	...
	public class BattleEnd extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			broadCastPacketToBothTeams(new ExShowScreenMessage("Р’СЂРµРјСЏ Р±РёС‚РІС‹ РёСЃС‚РµРєР»Рѕ. РўРµР»РµРїРѕСЂС‚Р°С†РёСЏ С‡РµСЂРµР· 1 РјРёРЅСѓС‚Сѓ.", 4000, ScreenMessageAlign.BOTTOM_RIGHT, true));
			end();
		}
	}
	...
	private class TeleportListener implements OnTeleportListener
	{
		...
		public void onTeleport(Player player, int x, int y, int z, Reflection reflection)
		{
			...
			removePlayer(player, false);
			player.sendMessage("Р’С‹ РґРѕСЃСЂРѕС‡РЅРѕ РїРѕРєРёРЅСѓР»Рё Р·РѕРЅСѓ Р±РёС‚РІС‹ Рё Р±С‹Р»Рё РґРёСЃРєРІР°Р»РёС„РёС†РёСЂРѕРІР°РЅС‹.");
		}
	}

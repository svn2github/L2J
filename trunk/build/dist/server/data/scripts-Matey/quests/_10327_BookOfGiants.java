...
public class _10327_BookOfGiants extends Quest implements ScriptFile
{
	...
	{
		Functions.npcSayToPlayer(Tairen, st.getPlayer(), NpcString.ENOUGH_OF_THIS_COME_AT_ME);
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("attak"))
		{
			...
			if(Tairen != null)
			{
				...
				if(Rnd.chance(33))
					Functions.npcSayToPlayer(Tairen, st.getPlayer(), NpcString.LOOKS_LIKE_ONLY_SKILL_BASED_ATTACKS_DAMAGE_THEM);
				if(Rnd.chance(33))
					Functions.npcSayToPlayer(Tairen, st.getPlayer(), NpcString.YOUR_NORMAL_ATTACKS_ARENT_WORKING);
				if(Rnd.chance(33))
					Functions.npcSayToPlayer(Tairen, st.getPlayer(), NpcString.USE_YOUR_SKILL_ATTACKS_AGAINST_THEM);
			}
		}
		if(event.equalsIgnoreCase("spawnas"))
		{
			...
			Functions.npcSayToPlayer(Assassin1, st.getPlayer(), NpcString.FINALLY_I_THOUGHT_I_WAS_GOING_TO_DIE_WAITING);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == panteleon)
		{
			if(st.isCompleted())
				htmltext = "0-c.htm";
			else if(cond == 0 && st.getPlayer().getLevel() < 20)
				htmltext = "start.htm";
			else if(cond == 1)
				htmltext = "0-3.htm";

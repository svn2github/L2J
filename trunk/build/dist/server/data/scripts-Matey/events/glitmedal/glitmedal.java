...
public class glitmedal extends Functions implements ScriptFile, OnDeathListener, OnPlayerEnterListener
{
	...
	private static File[] multiSellFiles = {
		new File(Config.DATAPACK_ROOT, "scripts/events/glitmedal/502.xml"),
		new File(Config.DATAPACK_ROOT, "scripts/events/glitmedal/503.xml"),
		new File(Config.DATAPACK_ROOT, "scripts/events/glitmedal/504.xml"),
		new File(Config.DATAPACK_ROOT, "scripts/events/glitmedal/505.xml"),
		new File(Config.DATAPACK_ROOT, "scripts/events/glitmedal/506.xml"),
	};
	...
	public void onDeath(Creature cha, Creature killer)
	{
		...
		{
			long count = Util.rollDrop(1, 1, Config.EVENT_GLITTERING_MEDAL_MEDAL_DROP_CHANCE * killer.getPlayer().getRateItems() * ((MonsterInstance) cha).getTemplate().rateHp * 10000L, true);
			if(count > 0)
			{
				addItem(killer.getPlayer(), EVENT_MEDAL, count);
			}
			if(killer.getPlayer().getInventory().getCountOf(Badge_of_Wolf) == 0 && Rnd.chance(Config.EVENT_GLITTERING_MEDAL_GLITTERING_DROP_CHANCE * killer.getPlayer().getRateItems() * ((MonsterInstance) cha).getTemplate().rateHp))
			{
